...
- [scripts/consensus-rnd-cli runtime-retention](scripts/consensus-rnd-cli runtime-retention) — canonical RuntimeRetention helper for host-opt-in pending-events compaction and planner-proven stale worktree remove/prune; legacy `generated_files` entries are inert.
