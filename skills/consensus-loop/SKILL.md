...
| `scripts/codex_refactor_loop/controller_actions.py` + `scripts/codex_refactor_loop/review_gate_selection.py` + `scripts/codex_refactor_loop/review_evidence_recovery.py` | `review-pr<N>-<role>-r<R>` reviewer prompt/log filename identity, GitHub-visible final-sentinel review evidence selection, bounded `review-evidence-redispatch` recovery keys, and repeated same-head blocker projection | `controller_actions.py` is the canonical writer for #396 reviewer dispatch prompt/log artifact names; completed reviewer evidence authority is GitHub-visible same-head final-sentinel comments selected by `review_gate_selection.py`; planner/runner redispatch uses `review_evidence_recovery.py` keys `(pr, role, head, reason)` with cap 2, and same-head repeated review verdict signatures may produce status-only `repeated_review_blocker` / explicit-approval waits. Cross-head same-PR counts are diagnostic only. Local review artifacts are pending/debug inputs only. |
...
Authorization source: `skills/consensus-loop/authorizations/runtime-exceptions.md#default-issue-intake-claim-623`. `DefaultIssueIntakeAdmission` is the read-only admission proof for #623: it only reads live GitHub issue state, open managed work snapshot, daemon progress/pending spawn intent facts, skill-private claim cooldown state, and host knobs. `DefaultIssueIntakeClaim` remains the checked-in active-controller-owner-only helper for admitted open, non-PR, not-yet-managed GitHub issues. `DEFAULT_ISSUE_INTAKE_ENABLE` defaults enabled; false-like host values disable both `wakeup-plan` projection and helper execution before any GitHub side effect. `wakeup-plan` may project only the named `controller_action="apply_default_issue_intake_claim"` action after no higher-priority managed work is dispatchable and `DefaultIssueIntakeAdmission` proves active cap, cooldown, upstream-idle, and concrete-work-unit admission; `wakeup-runner` may consume it only through #396 closed action projection and must recompute the same admission before any comment or label write.

Allowed: active-controller owner only; read live issue state, open managed work, daemon progress/pending spawn intent facts, host knobs, skill-private last-claim cooldown state, and issue comments; parse `crnd:default-issue-intake-claim` comments; use the earliest valid claim comment's GitHub `createdAt` plus `author.login` only as first-claimer admission/accounting facts inside this comment protocol; when no earlier claim exists, the current authenticated actor may write one claim comment and apply the existing `labels.design_issue_label_bundle()` labels; when an earlier other actor claim exists, write only a `crnd:default-issue-intake-stop` stop comment. The design label bundle is the only label mutation and routes the issue into the existing managed design-solving path.

Forbidden: no `UNMANAGED_ISSUE_INTAKE_ENABLE`, `UnmanagedIssueIntakeClaim`, `intake_unmanaged_issue_claim`, `crnd:unmanaged-issue-intake-*`, public lifecycle CLI, new daemon, generic issue factory, issue/PR close/reopen/body/title edit, PR edit/merge, assignee, milestone, tag/release, commit/push, generic label mutation, takeover permit, per-work claim, lifecycle owner, lifecycle authority, daemon owner, #191 lease owner, prompt-body authorization, or any GitHub side effect that bypasses #191 and #396. `DefaultIssueIntakeAdmission` grants no issue/PR close/reopen/body/title edit, assignee, milestone, PR merge, tag/release, public lifecycle CLI, new daemon, generic label mutation, or bypass of #191/#396. GitHub username, authenticated actor, comment author, and issue author facts are admission/accounting/display metadata only within this surface and never durable owner identity. Verification: `test_default_issue_intake_admission.py`, `test_default_issue_intake.py`, `test_wakeup_plan.py`, `test_wakeup_runner.py`, `test_host_env_surface_matrix.py`, `test_runtime_exception_authorization_sources.py`, and `test_skill_reference_anchors.py`.

## Named runtime exception - wakeup-runner(per #396)
Authorization source: `skills/consensus-loop/authorizations/runtime-exceptions.md#wakeup-runner-396`. `wakeup-runner` is active-controller owner only and consumes only `wakeup-plan` evidence-bound closed action projection. valid `HARNESS_SPAWN_INTENT` ordinary execution is owned by #396 `wakeup-runner`: producers such as `phase9-router` and concurrency top-up write closed spawn-intent projection evidence, and the runner revalidates then calls the checked-in spawn helper. controller/harness direct spawn is only mechanical daemon-stuck fallback in [Daemon-first controller boundary](#daemon-first-controller-boundary). The effect-adapter boundary is only the owner-local admission contract before existing #396 controller actions/named helpers and the #403 `ControllerActions.apply_issue_decomposition_plan()` helper; it is no generic effect-adapter runtime abstraction, no public command bus, and no executor layer. Effects are allowed only by concrete `controller_action` or helper name. It revalidates #191 owner, durable artifact evidence, clean `EXIT=0` source marker when required, ConsensusGate/meta-judge or review truth table `reject==0 && approve>=1 && all required reviewers present && all required GitHub-visible final-sentinel reviewer comment heads equal live PR head`, target-required PR merge-readiness checks, OPEN/live GitHub state, missing/stale per-reviewer GitHub comment head SHA, release #322 preflight, safe-progress admission metadata, and helper-specific preconditions before mechanically calling existing controller helpers or the named #396 helpers. Medium risk actions must carry `risk_tier: "medium"` plus `execution_policy: "cautious"` and are bounded per tick; `risk_tier: "high"` or blocked execution policy is a schema violation. Ordinary rejection uses grep-able one-line runner diagnostics such as `WAKEUP_RUNNER_BLOCKED:<action_id>:<reason>` plus ledger/pending-event reason; duplicate or in-flight target-log spawn intents are diagnostic skipped actions, not fatal blocked actions, and the runner continues scanning later spawn actions in the same tick; actual spawn helper launch failures remain blocked. `--dry-run`/test-only runner execution must not append live wakeup-runner ledger rows, and legacy `status="dry-run"` rows have no retry-suppression authority. Multi-step external side effects require helper-owned durable result/diagnostic artifacts when that helper owns such a surface or partial external state can exist. `.refactor-loop/host.env` may be skill-private runtime/cache/log read state only, not branch topology, machine paths, durable ledger authority, host artifact, or host production SSOT. `wakeup-plan` action `head_sha` is not reviewer-head authority; local `.refactor-loop/runs/review-pr<N>-<role>-r<R>.md` and `.refactor-loop/logs/review-pr<N>-<role>-r<R>.log` files are diagnostics, prompt inputs, completion/pending hints, and stale redispatch hints only, not merge/fix lifecycle authority; raw PR-head check buckets or advisory check buckets are display-only diagnostics, not merge/fix lifecycle authority. Consensus→implement projection durable fact source is the consensus judge artifact frontmatter, `## If consensus`, `Implementation owner`, and Implement plan structured fields `scope_paths`, `old_pattern`, `new_principle`, and optional `verification_hints`; parser failure emits no implementation action. Consensus implementation readiness is a helper-specific precondition: dispatchable projection requires `consensus_implementation_ready` and suppresses to status-only with `suppressed_reason` such as `open_closing_pr`, `remote_iter_branch`, `in_flight_implement`, or `scope_conflict_waiting` when the OPEN managed issue is already claimed by a closing PR, canonical implementation branch/worktree, implement log, pending intent, in-flight worker, or an earlier executable consensus implementation action with overlapping normalized `scope_paths`; runner and controller helper revalidate before dispatch. `crnd:triage:resume-requested` is a label-catalog owned maintainer/cockpit directive that may only project into this existing `dispatch_consensus_implementation` action for a live OPEN managed issue with the label still present, a latest durable consensus implementation judge artifact, and readiness passing; runner revalidates the live label/managed/open state, and successful helper phase transition clears only that resume request label as part of existing issue label cleanup. It is not standalone authorization, not a generic resume ticket, not a public CLI/lifecycle actor, and not authority for issue/PR close/reopen/body/title/merge/tag/release actions. `dispatch_consensus_implementation` only moves the issue to implementing phase, creates the canonical implementation worktree/head, renders the implement prompt, and spawns the implement worker; it does not commit, push, or open a PR. For implementation publish, PR title/body are worker-authored GitHub-facing artifacts: title SSOT is `.refactor-loop/runs/implementation-pr-${CLUSTER_ID}-title.txt`, body SSOT is `.refactor-loop/runs/implementation-pr-${CLUSTER_ID}-body.md`; `wakeup-plan` projects only these paths and suppresses missing or invalid artifacts, while `wakeup-runner` and `ControllerActions.publish_implementation_output` revalidate path containment, sentinel, exactly one matching `Closes #N`, required sections, and non-placeholder title/body. After validating a real scoped diff, committing it, and restoring the integration base, `publish_implementation_output` runs helper-private publish verification before `safe_push`; that detached/resumable gate records same issue/action/head_ref/worktree realpath/exact head SHA/BUILD+TEST command digest plus `EXIT=0` evidence in `.refactor-loop/state/publish-verification/` and fails closed before push when evidence is missing, mismatched, or failed. Only then may it safe-push the canonical head, update an existing matching open managed implementation PR when exactly one exists, or open exactly one managed implementation PR and verify it when zero matching PRs exist; exactly one matching open managed implementation PR whose remote head is proven equal to canonical worktree `HEAD` is already complete, so `wakeup-plan` emits `status_only` with `suppressed_reason=pr_already_open_current` and a stale helper invocation returns 0 before diff/build/test/push/PR edit/reviewer dispatch. Remote ref proof is read-only and bounded; unknown or not-current evidence falls through to the existing publish path and is not success authority; duplicate/multiple PRs, head/base/link mismatch, unmanaged PRs, invalid worker-authored PR artifacts, or stale-base clean output fails closed/status-only, and stale-base clean `IMPLEMENT_DONE:ok` is `implementation_refresh_needed:stale_base` without clearing the log. Clean `IMPLEMENT_DONE:ok` with an empty scoped diff is a no-op publish input: `wakeup-plan` suppresses it to status-only and `wakeup-runner` records stale executable projections as skipped rather than blocked, with no close/merge/label side effect. Before runner application, `wakeup-plan` prunes stale, terminal, or superseded local evidence from its projection; release-rollup freshness may use read-only local `refs/remotes/origin/<review_base>..refs/remotes/origin/<integration>` evidence to suppress stale pending events, and local ref probe failure fails open. This narrows runner input only and does not weaken #396 revalidation or create standalone authorization. Allowed actions are spawn codex from valid `HARNESS_SPAWN_INTENT`, including allowlisted `release-rollup-body` generation that only writes `.refactor-loop/runs/release-rollup-pr-body.md`, named helper `dispatch_consensus_implementation`, named helper `publish_implementation_output`, named helper `apply_issue_decomposition_plan`, named helper `apply_default_issue_intake_claim`, then named helper `open_release_rollup_pr_from_action` after the body exists, publish worker output, dispatch reviewers/fix/remote-ci worker only for target-required failed checks with `target_required_checks_red`, apply triage decision, merge PR under review truth table plus target-required readiness, close managed item from drop marker, and publish release through #322. Forbidden: no arbitrary git/gh command, workflow tag/release, router guard adjudication, generic codex fallback, prompt-body decision, standalone authorization from `wakeup-plan`; the fixed forbidden field set is at least `cmd`, `argv`, `shell`, `command_line`, `commands`, `env`, `git`, `gh`, `executor`, `lifecycle_authority`, and `lifecycle_owner`, with existing extra `args` rejection retained; no generic command fields, new lifecycle authority, `ControllerTurnDecision`, controller-turn worker, private schema, active-active scheduler, `.refactor-loop/host.env` as host production SSOT, generic lifecycle actor, and arbitrary label/merge/close outside existing helper or named #396 helper. Verification: `test_wakeup_runner.py`, `test_wakeup_runner_review_gate.py`, `test_wakeup_runner_release.py`, `test_wakeup_plan.py`, `test_safe_progress_scheduler.py`, `test_controller_actions.py`, `test_cli_command_router.py`, `test_runtime_exception_authorization_sources.py`, `test_restart_daemons.py`, and `test_skill_reference_anchors.py`.

No-change/false-positive consensus defer is named helper `defer_false_positive_consensus`: `wakeup-plan` may project `controller_action="defer_false_positive_consensus"` only from a clean consensus marker plus durable judge artifact identity, normalized `scope_paths: none`, no-change/false-positive framing, and live OPEN managed design-solving issue. The runner revalidates those facts, and the helper only posts the fixed explanation and transitions labels to `crnd:phase:blocked` plus `crnd:human:auto`; non-empty `scope_paths` consensus continues to `dispatch_consensus_implementation`. It has no close/reopen/body/title/PR/git/release/generic command authority.

<a id="named-runtime-exception--runtime-retentionper-437"></a>
## Named runtime exception - RuntimeRetention(per #437)
Authorization source: `skills/consensus-loop/authorizations/runtime-exceptions.md#runtime-retention-437`. `RuntimeRetention` is the canonical owner for skill-private local runtime cleanup. `consensus-rnd-cli runtime-retention` is the canonical command and the only public command for this surface.

Narrow allowlist: only when `$RUNTIME_RETENTION_ENABLE=true`, RuntimeRetention never deletes `$REPO_ROOT/.refactor-loop/{logs,prompts,runs}` worker artifacts. Legacy `generated_files` entries in `.refactor-loop/state/runtime-retention-plan.json` are compatibility input only: they are ignored, counted in `kept`, and grant no delete authority. RuntimeRetention compacts `$REPO_ROOT/.refactor-loop/.controller-pending-events.log` and `$REPO_ROOT/.refactor-loop/.concurrency-alert.log` in place so Monitor bridges keep the same inode; consumes stale `$REPO_ROOT/.worktrees/<name>` entries present in the same plan with proof fields `no_in_flight`, `no_open_issue_or_pr`, `no_dirty`, `no_local_ahead`, and `merged_or_missing_safe`, after local git read rechecks pass; and may unlink only regular `$REPO_ROOT/.refactor-loop/locks/spawn-tasks/<safe-task-id>.lock` files when metadata is valid and matches the basename, `log_path` resolves under `$REPO_ROOT/.refactor-loop/logs/`, TTL has elapsed, and the matching log exists with a terminal `EXIT=` marker. It may then run `git worktree remove <path>` and `git worktree prune` for eligible worktrees. Missing/false opt-in exits 0 with a noop summary.

Forbidden: no GitHub write or lifecycle authority, no `git fetch`, branch deletion, commit, push, reset, rebase, merge, tag, release, archive/index durable fact source, host config edit, host production SSOT under `.refactor-loop/host.env`, generic lifecycle actor, compatibility alias, `.refactor-loop/{logs,prompts,runs}` worker artifact deletion, `generated_files` deletion authority, `RuntimeRetentionPlan.spawn_task_locks`, missing-log companion/plan proof, dead-holder/no-in-flight/GitHub terminal-state lock proof, path-escaped lock logs, or worktree cleanup without planner proof. Verification: `test_runtime_retention.py`, `test_cli_command_router.py`, `test_restart_daemons.py`, `test_runtime_exception_authorization_sources.py`, and `test_skill_reference_anchors.py`.

<a id="large-issue-decomposition"></a>
## Large issue decomposition(per #403)
Authorization source: `skills/consensus-loop/authorizations/runtime-exceptions.md#issue-decomposition-403`. Large or epic issue decomposition is allowed only after design-consensus/meta-reflector consensus that the source is scope-too-broad or explicitly `decompose`. The #403 admission boundary is the existing `ControllerActions.apply_issue_decomposition_plan()` helper's private validation gate, not a second apply schema, public command bus, executor layer, or generic effect-adapter runtime abstraction. The only controller-private handoff is an `IssueDecompositionPlan` artifact with exactly `{schema, parent_issue, source_consensus_artifact, children:[{slug,title,scope,non_goals,body_artifact_path}], parent_update:{comment_artifact_path}}`. The plan must not contain lifecycle owner/authority, command, argv, args, shell, command_line, commands, env, gh, git, executor, close, assignee, milestone, proof, digest, plan digest, controller action, kind, absolute path, or path traversal fields; child bodies must be self-contained GitHub bodies with parent issue, source consensus artifact, scope, non-goals, and final sentinel.

Only the active-controller owner may run the checked-in internal `ControllerActions.apply_issue_decomposition_plan()` helper after `issue_decomposition.py` validates the plan. The helper builds apply state from exact helper-owned parent tracking comments plus child fingerprint discovery through the bounded `ManagedWorkSnapshot` read model; unavailable or too-stale snapshot discovery under low GraphQL headroom is a grepable non-consuming backoff, not a hard failure or ledger-consuming apply. The snapshot is discovery-only and is not GitHub live-state authority, lifecycle permit, or host production SSOT; immediately before creating a missing child, the helper performs a narrow live duplicate check for that child fingerprint under the existing #191 owner gate. The helper derives each child idempotency fingerprint from parent issue, plan digest, and child slug, creates only missing child design issues through the catalog design issue label bundle (`crnd:lifecycle:managed`, `crnd:phase:design-solving`, `crnd:human:auto`), and writes a consolidated parent tracking comment with the exact helper-owned grammar. Duplicate matching parent tracking comments reconcile deterministically; malformed, conflicting parent issue/digest, unknown child slug, or mismatched child fingerprint data fails closed. This is not a public issue factory, generic effect adapter, command bus, or executor layer. The parent epic remains open/tracking: no parent issue close, reopen, body edit, title edit, assignee, milestone, or label lifecycle mutation is part of #403.

Discoverability stays on existing surfaces. `META_JUDGE_DONE:consensus:decompose` is not a phase9 direct route; the router appends an existing-format `phase9-router-fallback` pending event. A clean judge log remains visible through `completed_marker_actions()` as `kind: completed-marker`, `phase: design-consensus`, `actor: design-consensus-router-or-controller`. The first `consensus:decompose`, solver artifacts, prompt body, validator result, worker output, and `.refactor-loop/host.env` are not apply authorization. Only when the plan-level judge artifact structured fields carry the exact named `controller_action="apply_issue_decomposition_plan"`, `plan_level_design_consensus_judge_artifact`, plan path, digest, and proof does `wakeup_plan.py` project that single #396 closed action; `wakeup_runner.py` then revalidates clean plan-level judge source marker, digest/proof, live parent open/tracking, forbidden fields, and exact helper-owned tracking grammar before calling the #403 helper, without treating loose sentinel-looking prose as idempotency state. `wakeup_plan.py` is not the #403 read-model/status/authorization owner and must not project any other decompose action/status, `kind="issue-decomposition-apply"`, private dialect, public issue factory, or generic lifecycle action.
...
Authorization source: `skills/consensus-loop/authorizations/runtime-exceptions.md#closed-label-reconciler-238`. This is the only closed managed item phase-label reconciliation carveout. Consensus marker: `META_JUDGE_DONE:consensus:closed-label-reconciler:option B named restart-managed closed-label-reconciler with crnd:phase:closed and gh-label-closed-reconcile authority`.
...
`skills/consensus-loop/scripts/consensus-rnd-cli statusline` 是 fast (<200ms) read-only Claude Code statusline reader,显示本仓库 loop 实时状态(codex 计数、PR/issue 数、daemon 健康、P0 streak、freeze 指示、optional update notice)。
...
授权来源:`skills/consensus-loop/authorizations/runtime-exceptions.md#statusline-51`(Consensus-rnd Phase design-consensus r3 3/3 unanimous consensus on C framing)。
...
`skills/consensus-loop/scripts/consensus-rnd-cli restart-daemons` 是 checked-in,host-agnostic restart helper。It owns singleton wrappers + actor-owned heartbeat leases + helper-private launch fingerprints for the static daemon allowlist(`concurrency_monitor`, `comment-monitor`, `codex-progress-reporter`, `dev_sync_daemon`, `phase9_router_daemon`, `closed_label_reconciler`, `wakeup_runner_daemon`)。Each wrapper may self-heal only its own static-allowlist child on child exit or missing/malformed/stale actor heartbeat by reading `.refactor-loop/heartbeats/<daemon>.ts`, logging one reason line, terminating that child with SIGTERM/grace/SIGKILL if still alive, and restarting the same resolved command; the wrapper does not write heartbeat and does not reload fingerprint or host.env. Each daemon actor writes canonical tick progress to `.refactor-loop/state/daemon-tick-progress/<daemon>.json`: a tick starts as `begin`, ends as `complete` or `fail`, and only a successful `complete` tick is followed by the actor heartbeat renewal. `restart-daemons` remains the only owner of wrapper singleton, managed child command instance cleanup, bounded lock-holder cleanup, fingerprint/env reload, boot/post-wake repair, active-controller owner gate, and old wrapper replacement; the controller's periodic wakeup is the primary supervisor that invokes it, and cron/launchd is an optional outer keepalive only for unattended operation with no controller session. 事实源是 `.refactor-loop/locks/<daemon>.pid`、`.refactor-loop/heartbeats/<daemon>.ts`、`.refactor-loop/state/daemon-tick-progress/<daemon>.json`、`.refactor-loop/locks/<daemon>.fingerprint.json`,以及 helper-private `DaemonProcessInventory` / `DaemonInstanceProjection`;只有 pid alive、actor-loop heartbeat fresh(`<90s`)、current fingerprint、held valid singleton lock by the live managed child、同一 repo_root plus daemon name plus restart wrapper shape plus same resolved static allowlist command 零 duplicate canonical live wrapper、且零 orphan bounded lock holder 时才 skip。Progress freshness uses `DaemonProgressBudget` from the restart-managed `DaemonRuntimePolicy`: completed ticks stay fresh through resolved tick interval plus restart progress grace, while begin ticks use only restart progress grace; `daemon-status` and `restart-daemons` call the same budget helper. missing/malformed/mismatch fail-closed, begin-overdue/failed/stale/missing progress fail-closed even when heartbeat is fresh, duplicate canonical wrappers fail closed, and orphan bounded lock holder 也 fail closed:启动或修复前 reap 全部同仓库同 daemon 旧 wrapper 和 projection-proven bounded lock-holder child,随后只留下一个 fresh wrapper。Actor-held singleton lock 是 #49 局部 owner-local runtime coordination fact,不是 `zero duplicate canonical live wrapper` 的替代项,也不是通用 daemon registry 或 #191 lease 替代。`DaemonRuntimePolicy` only mirrors `restart.py::DAEMON_COMMANDS` owner-local target cadence; it is not a daemon registry, lifecycle authority, host production SSOT, or #191 lease substitute. `DaemonLaunchFingerprint` writes `host_env_path` and `host_env_sha256`; those fields are local reload invalidation and read-only stale projection only, never host.env plaintext, host production SSOT, branch topology, durable ledger, host artifact authority, or side-effect authorization. 每次 helper tick 先调用 canonical `consensus-rnd-cli runtime-retention`;不保留兼容 alias。
Actor-held singleton locks are an owner-local runtime coordination surface for restart-helper-managed daemon lifetimes. Allowed: a daemon actor that was launched by `restart-daemons` may hold an open `fcntl.flock` descriptor on `.refactor-loop/locks/<daemon>.singleton.lock`, write one JSON metadata record, and require that lock before actor heartbeat writes or lease sleeps. `restart-daemons` and `daemon-status --json` may probe that file as local mutual-exclusion evidence for the same static daemon target. Forbidden: the singleton lock must not create, close, reopen, edit, merge, label, commit, push, tag, release, spawn codex, authorize lifecycle side effects, replace the #191 active-controller lease, become a host production SSOT, or act as a generic daemon registry. Fact source: the live open file lock is the mutual-exclusion fact; the file body is diagnostic metadata only. Canonical metadata field order is `daemon_name`, `repo_root`, `actor_pid`, `heartbeat_file`, `fingerprint_file`, `command_sha256`, `started_at`, then `lock_generation`. Allowed characters: `daemon_name` is ASCII alphanumeric plus `_` and `-`; `repo_root` is the literal `$REPO_ROOT`; `heartbeat_file` and `fingerprint_file` are printable repo-symbolic `$REPO_ROOT/...` paths with no control characters or `..` segments; `command_sha256` is empty or lowercase 64-hex; status/reason values are the literal ASCII tokens listed here; exported `singleton_lock_path` is a printable repo-symbolic `$REPO_ROOT/.refactor-loop/locks/<daemon>.singleton.lock` string. String fields must be non-empty except `fingerprint_file` and `command_sha256`, integer fields must be positive, `lock_generation` is written as exactly `1`, and canonical writes emit compact JSON in the documented field order plus trailing newline from the actor holding the lock. Status states are exactly `missing`, `free`, `held`, `held-malformed`, and `probe-error`; `held-malformed` means another process holds the lock but metadata cannot validate, so restart-daemons must skip termination and launch while daemon-status reports stale diagnostics. Legacy read/migration policy: missing singleton locks are `missing` and old fingerprint/heartbeat/pid facts remain readable; free valid metadata is diagnostic only and does not block restart; malformed unlocked files are not holder evidence. Runtime inputs `RESTART_DAEMON_SINGLETON_LOCK`, `RESTART_DAEMON_SINGLETON_LOCK_FILE`, `RESTART_DAEMON_FINGERPRINT_FILE`, and `RESTART_DAEMON_COMMAND_SHA256` are written only by `restart-daemons` for the child environment and are not host.env public knobs. `daemon-status --json` may publish only the parsed status fields `singleton_lock_path`, `singleton_lock_state`, `singleton_lock_holder_pid`, `singleton_lock_metadata_valid`, and `singleton_lock_reason`.
...
`consensus-rnd-cli daemon-status --json` is the paired read-only daemon-status projection. It requires the same base host admission needed to compare launch identity: `CONSENSUS_RND_HOST_ENV` must name a readable host-owned `host.env`, parsed host.env must be non-empty, host.env `REPO_ROOT` must match the loaded repository, and `GH_REPO_SLUG` must be in `OWNER/REPO` form. Admission failure exits 2 with `daemon-status requires CONSENSUS_RND_HOST_ENV ...` or another command-specific diagnostic before reading daemon inventory, stored fingerprints, or printing JSON; it must not fabricate stale JSON with `host_env_path=""` / `host_env_sha256="missing"` and must not add an `unknown` daemon-status schema state. After admission, it reports `running`, `stale`, `dead`, or `not-owner` from the existing static allowlist, helper-private launch fingerprint, pid/heartbeat/progress readers, cached active-controller status, and `DaemonProcessInventory`, including stale reason/age, `progress_status`, `progress_age_seconds`, `progress_reason`, and duplicate count. It uses the same split progress budget helper as `restart-daemons`: completed ticks are fresh through `tick_interval_seconds + RestartConfig.progress_fresh_seconds`, while begin ticks use `RestartConfig.progress_fresh_seconds`. Fresh heartbeat alone is not `running`: duplicate wrappers, orphan bounded lock holders, missing progress, in-progress overdue ticks, failed progress, truly overdue completed progress, and fingerprint mismatch report stale so `restart-daemons` can repair/reload. It has no public start/stop/restart/reload lifecycle verb. Repair/reload remains restart-daemons.

完整下游装机顺序见 [Downstream install walkthrough](#downstream-install-walkthrough);本段保留 scheduler-backed helper invariant and the optional cron/launchd outer repair surface for unattended operation; the controller's periodic wakeup is the primary supervisor and a stable loop rarely needs restart.
...
`skills/consensus-loop/scripts/consensus-rnd-cli restart-daemons` = Consensus-rnd Phase design-consensus r3 授权的 scheduler-backed anti-stop helper,不新增 watchdog daemon。

- **Narrow allowlist**: helper owns singleton wrappers + actor-owned heartbeat leases + helper-private launch fingerprints for `concurrency_monitor`, `comment-monitor`, `codex-progress-reporter`, `dev_sync_daemon`, `phase9_router_daemon`, `closed_label_reconciler`, and `wakeup_runner_daemon` in the existing static daemon allowlist;each wrapper may self-heal only its own child on child exit or missing/malformed/stale actor heartbeat by reading the actor-owned heartbeat and restarting the same resolved command;heartbeat 是 actor-loop progress lease boundary signal,not a wrapper sidecar liveness proof;canonical `DaemonTickProgress` is the progress fact source, and daemon actors write `begin` / `complete` / `fail`, renew heartbeat only after successful `complete`, and keep failed or overdue progress visible for repair/reporting;split progress budgets come from the restart-managed `DaemonRuntimePolicy`, where completed ticks use resolved tick interval plus restart progress grace and begin ticks use only restart progress grace;fingerprint artifact `.refactor-loop/locks/<daemon>.fingerprint.json` 只记录 daemon name、resolved command、CLI entrypoint hash、Python package tree hash、文件计数、`host_env_path` 和 `host_env_sha256`,只用于 restart skip eligibility and read-only stale projection;it never stores host.env plaintext and never becomes host production SSOT, branch topology, durable ledger, host artifact authority, or side-effect authorization;`DaemonProcessInventory` owns the restart-helper-local `DaemonInstanceProjection` and only matches same repo_root / daemon target / known command shape facts: canonical restart wrappers, managed child command instances, and bounded lock-holder evidence from target-owned local lock files. missing/malformed/mismatch fail-closed,发现 duplicate canonical live wrapper、orphan bounded lock-holder child、or missing/failed/overdue/stale progress 时 fail closed even with fresh heartbeat:启动或修复前 terminate 全部同仓库同 daemon 旧 wrapper 和 projection-proven bounded lock-holder child,随后只 spawn 一个 fresh wrapper;the controller's periodic wakeup is the primary supervisor that invokes restart-daemons, and cron/launchd is an optional outer keepalive only for unattended operation with no controller session covering wrapper death, boot/post-wake, fingerprint/host.env reload, duplicate cleanup, bounded lock-holder cleanup, progress-overdue repair, and #191 owner refresh;并顺手运行 canonical `consensus-rnd-cli runtime-retention`;不 spawn codex / commit / push / merge / label / archive。
- **Read-only status projection**: `consensus-rnd-cli daemon-status --json` first requires explicit host-owned `CONSENSUS_RND_HOST_ENV` admission so launch fingerprint/currentness facts are comparable. Missing, empty, unreadable, unparsable, repo-root-mismatched, or invalid-slug host.env fails closed with exit 2 before process inventory, stored fingerprint reads, or JSON output; it does not project fabricated stale JSON and does not add an `unknown` schema state. After admission, it mirrors the same static allowlist and helper-private pid/heartbeat/progress/fingerprint/inventory facts plus cached active-controller status, and projects duplicate count, managed child command pids, bounded lock-holder pids, `progress_status`, `progress_age_seconds`, and `progress_reason` from the same `DaemonInstanceProjection` plus `DaemonTickProgress` and the same split `DaemonProgressBudget` helper as `restart-daemons`. Fresh heartbeat plus no wrapper plus orphan bounded lock-holder child is `stale` with grepable `orphan-lock-holders:N` diagnostics, not `running`; fresh heartbeat plus failed/overdue/missing progress is also `stale` with grepable `progress-*` diagnostics. It is read-only status only, has no public start/stop/restart/reload lifecycle verb, and repair/reload remains restart-daemons.
...
- **Behavior tests**: `test_restart_daemons.py` 覆盖 fresh heartbeat + matching fingerprint skip / stale/missing/malformed heartbeat repair / held-malformed singleton lock skip without reaping or launching / free singleton metadata diagnostic-only restart / dead pid repair / missing/malformed/mismatched fingerprint restart / CLI entrypoint and package tree fingerprint restart / duplicate cleanup / concurrent helper no double-spawn / deterministic hung actor restart;`test_daemon_heartbeat.py` 覆盖 deterministic lease sleep renewal and actor-held singleton lock states;`test_daemon_status.py` 覆盖 singleton status fields including free metadata projection;`test_runtime_retention.py` 覆盖 worker artifact retention / legacy generated_files inert handling / pending-events/concurrency-alert compaction / stale worktree proof gate / restart hook。
- **Source-regression**: `AntiStopRestartHelperContractTests` + `RestartDaemonsBehaviorTests.test_restart_helper_source_mentions_launch_fingerprint_contract` + `RuntimeRetentionSourceRegressionTests` 字面断言本段标题、narrow allowlist、no lifecycle authority、cron/launchd install、#49 mirror authorization path、helper singleton check + actor-owned heartbeat freshness check + helper-private launch fingerprint fact source、actor-held singleton lock contract、controller wakeup ordering、anti-regression forbidden tokens、legacy generated_files inert handling、no wrapper sidecar heartbeat writer、direct rm、no archive/index/new daemon。
...
Authorization source: `skills/consensus-loop/authorizations/runtime-exceptions.md#controller-tick-supervisor-553`.
...
`ControllerTickSupervisor` is a narrow migration scheduler for named `TickHandler` implementations. Each migrated handler has a supervisor-local `TickHandlerContract` naming only handler identity, required projection sources/freshness, delegated existing helper, replaced legacy daemon target, and net-deletion target; contract fields carrying `argv`, `shell`, `cmd`, `command_line`, `commands`, `env`, `git`, `gh`, `executor`, `owner`, `pending_events_authority`, `lifecycle_authority`, or `lifecycle_owner` fail closed. It consumes only `TickWorkItem(handler,key)` pairs from `KeyOnlyWorkQueue`; queue items carrying command or lifecycle fields fail closed before dispatch. Handler `backoff`, `blocked`, and `noop` results are returned and logged as diagnostics only; they are not git, gh, command, or lifecycle actions. `LegacyDaemonModeGuard` must prove the same target is not also owned by a legacy restart-helper-managed daemon before a handler runs. `$CONTROLLER_TICK_SUPERVISOR_ENABLE=true` opts `restart-daemons` into maintaining the supervisor target and mechanically excludes the migrated `comment-monitor` legacy daemon target from the active restart inventory; false or empty leaves the canonical legacy daemon list unchanged, including the canonical eight legacy daemon names.

First migrated target: `comment-monitor`. Its handler delegates to the existing `run_comment_monitor_reconcile_tick()` / `CommentMonitor.tick()` path, so active-controller ownership, GitHub authenticated actor admission, maintainer whitelist, content-creation backoff, AI-comment filters, and pending-event writes stay inside the existing comment-monitor helper. The handler requires fresh `managed_work_snapshot` projection evidence and returns `backoff` or `blocked` instead of executing when required projection freshness fails. `phase9-router` and `dev-sync` are not first migration targets.

Forbidden: no generic executor, no pending-events authority movement, no host production SSOT, no issue/PR lifecycle, no label mutation, no commit, no push, no merge, no tag, no release, no public lifecycle verb, no phase9-router migration, no first-pass dev-sync migration, and no write side-effect authorization movement. Any write side effect remains inside existing helpers, which must re-check #191/#396/#238/#322/#403/#437/#504 or #53 gates.
...
9. arm persistent daemon-event Monitor bridge for `.refactor-loop/.controller-pending-events.log` and `.refactor-loop/.concurrency-alert.log` through `consensus-rnd-cli monitor-bridge-filter`; it drops empty lines, `tail -F` headers, `HARD_GATE:dispatch_required`, pending P0 mirrors, known terminal runner blockers, and repeated raw P0 streaks except first/escalation lines.
...
| `IMPLEMENT_DONE:ok` | The only publishable implement marker. Controller verifies worker-authored `.refactor-loop/runs/implementation-pr-${CLUSTER_ID}-title.txt` and `.refactor-loop/runs/implementation-pr-${CLUSTER_ID}-body.md`; after real scoped diff validation, commit, and integration-base restore, `publish_implementation_output` runs helper-private publish verification for the same issue/action/head_ref/worktree realpath/exact head SHA/build-test command digest and required `EXIT=0` evidence before `safe_push`; then it updates exactly one matching open managed implementation PR or opens and verifies exactly one managed implementation PR when zero matching PRs exist, then confirms Consensus-rnd Phase review-gate reviewer dispatch evidence. If the exactly matching open managed PR already points at canonical worktree `HEAD`, `wakeup-plan` emits `suppressed_reason=pr_already_open_current` status-only and the helper no-ops before heavy work; Remote ref proof is read-only and bounded. Missing or invalid PR artifacts, duplicate/multiple PRs, conflict PRs, head/base/link mismatch, unmanaged PRs, failed or mismatched publish verification, unknown/not-current PR head evidence, or stale-base clean output fail closed/status-only or fall through to the existing publish path. |
| `IMPLEMENT_DONE:partial` / `IMPLEMENT_DONE:blocked` | Non-publishable `non_ok_marker` retry evidence. Fresh markers wait for `$STALE_REVIVAL_HOURS`; stale markers with no canonical in-flight worker may be cleared by existing redispatch cleanup and routed through existing `dispatch_consensus_implementation`. They do not publish, zero-code close, apply decomposition, or satisfy the expected implement worker. |
...
Authorization: `skills/consensus-loop/authorizations/runtime-exceptions.md#maintainer-directive-milestone-priority`.
...
`skills/consensus-loop/scripts/consensus-rnd-cli concurrency` 的 `top_up_from_dispatch_queue` + tick() deficit 分支 = Consensus-rnd Phase design-consensus / maintainer-directive 等价授权的 controller-runtime intent dispatch 路径(narrow allowlist):
...
-->
`HARNESS_SPAWN_INTENT.command` is exactly `"spawn-codex"` as a closed semantic enum, not argv and not shell. Valid intents carry `controller_action: "spawn_codex_harness_background"`, repo-relative canonical `cd` / `prompt` / `log` writer paths, positive integer `stall`, `run_in_background_required: true`, and `no_lifecycle_authority: true`. The owner-local consumer validator also accepts existing repo-contained absolute `cd` / `prompt` / `log` compatibility input; relative paths resolve under `$REPO_ROOT`, absolute paths must remain inside `$REPO_ROOT`, and this compatibility read does not add producer canonical-write migration or lifecycle authority. Writers and host specs must not add `argv`, `args`, `shell`, `cmd`, `commands`, `env`, `git`, `gh`, `executor`, `target_ref`, `ControllerCommand`, `ControllerEvent`, `SpawnIntentInbox`, `spawn-intents`, or any generic command bus. `wakeup-plan` projects valid intents into #396 runner actions and rejects invalid variants; the concurrency monitor calls the same validator after JSON object and timestamp parsing before treating pending intents as transient supply, rather than redefining malformed path or command semantics. `wakeup-runner` is the ordinary consumer that invokes the checked-in spawn helper; controller/harness argv construction is only the stuck-gated fallback.
...
授权来源:`skills/consensus-loop/authorizations/runtime-exceptions.md#maintainer-directive-concurrency-auto-topup`(per CLAUDE.md maintainer-directive equivalence 子句,PR #48 merged)。

## Named runtime surface — codex-progress-reporter TEST_NO_LOOP(per #69)

`skills/consensus-loop/scripts/consensus-rnd-cli progress-reporter` supports `TEST_NO_LOOP=1` only as a source-time test seam for `scripts/test_progress_reporter.py`.
...
- **Verification**: `python3 -m unittest skills/consensus-loop/scripts/test_progress_reporter.py` covers no per-worker progress comment mutation/read and #504 fixed-card PATCH behavior; `python3 -m unittest discover -s skills/consensus-loop/scripts -p 'test_*.py'` includes source-regression assertions for this narrow surface.

授权来源:`skills/consensus-loop/authorizations/runtime-exceptions.md#maintainer-directive-progress-reporter-orphan-delete` is obsolete/deleted by #626 and grants no recurring daemon delete path。
...
4. Truth table: `reject=0`, `approve=R`, `comment=0` → `MERGE`; `reject=0`, `approve>=1`, `comment>=1`, `approve+comment=R` → `MERGE_WITH_COMMENTS`; `reject=0`, `approve=0`, `comment=R` → `WAIT_EXPLICIT_APPROVAL`; `reject>=1` → `FIX`; missing role, duplicate/unknown verdict, missing final sentinel, missing/stale per-reviewer head SHA, CI pending/fail, or non-mergeable PR → `WAIT_OR_REDISPATCH`.
   `review-evidence-redispatch` may dispatch only roles still missing GitHub-visible same-head final-sentinel evidence and not already pending for that head. `CONFLICTING`/`DIRTY` PRs are status-only `blocked:needs-conflict-resolution`; each `(PR, role, head, invalid-reason-family)` recovery key is capped at two applied attempts, with skipped/blocked rows not consuming the cap.
...
`ConsensusGateProof` is a controller-private pure proof-validity contract for controller-private ConsensusGateProof per #579, implemented by `scripts/codex_refactor_loop/consensus_gate.py`. Authorization source: `skills/consensus-loop/authorizations/runtime-exceptions.md#consensus-gate-proof-579`. It is not a public CLI, runtime exception, wakeup action, proof-ticket/resume system, command bus, or lifecycle authority. It validates proof validity for substantive decision targets only: a stable non-mechanical `target_kind`, `target_ref`, `target_digest`, independent `decision_producer_id`, `evidence[]` entries with `producer_id`, `role`, `artifact`, `artifact_digest`, and `verdict`, plus `required_roles`, `verdict_rule`, and optional repo-relative `scope_paths`.

The pure validator rejects single-worker self-certification, target digest mismatch, target kind/ref mismatch, artifact digest mismatch, missing required roles, duplicate roles, duplicate or overlapping producers, unsupported verdict rules, non-positive required verdicts when the selected rule requires approval, mechanical target kinds, missing required `scope_paths`, and recursive lifecycle or command fields. The fixed forbidden field set includes `cmd`, `argv`, `shell`, `command_line`, `commands`, `env`, `git`, `gh`, `executor`, `lifecycle_authority`, `lifecycle_owner`, `args`, `controller_action`, `proof_ticket`, and `resume_ticket`.

Validated `ConsensusGateProof` only proves proof validity. Mechanical route/post/label/spawn/merge/apply-validated-proof actions do not recursively require `ConsensusGateProof`, and valid proof does not authorize GitHub, git, file lifecycle, route/post/label/spawn/merge, or apply side effects. A consuming helper may use valid proof only as an admission fact for its already-named helper path, then must still revalidate its own durable target digest, artifact digests, #191 active-controller owner gate, live GitHub state, and helper-specific preconditions before any effect. Verification lives in `test_consensus_gate.py`, `test_wakeup_plan.py`, `test_wakeup_runner.py`, `test_runtime_exception_authorization_sources.py`, and `test_skill_reference_anchors.py`.

Forbidden boundary: no GitHub/git/file lifecycle authority, no route/post/label/spawn/merge/apply side effects, no public CLI, no wakeup-plan action projection, no wakeup-runner action, no IssueDecompositionPlan apply migration in this issue, no standalone wakeup-plan action projection, no standalone wakeup-runner action, no proof-ticket/resume system, and no command bus.
...
- [scripts/consensus-rnd-cli runtime-retention](scripts/consensus-rnd-cli runtime-retention) — canonical RuntimeRetention helper for host-opt-in pending-events and concurrency-alert compaction, planner-proven stale worktree remove/prune, and log-`EXIT=` proven spawn-task lock unlink; legacy `generated_files` entries are inert.
...
5. `IMPLEMENT_DONE:ok` means the controller may inspect diff, commit, restore integration base, run helper-private publish verification for exact head/build-test `EXIT=0` evidence, safe-push the canonical head, then have `publish_implementation_output` open or update exactly one managed implementation PR; duplicate/multiple/mismatch/unmanaged PRs fail closed.
...
`consensus-rnd-cli restart-daemons` also runs daemonless RuntimeRetention before daemon freshness checks. `consensus-rnd-cli runtime-retention` has no GitHub lifecycle authority and is disabled unless `$RUNTIME_RETENTION_ENABLE=true`. When enabled, RuntimeRetention never deletes `$REPO_ROOT/.refactor-loop/{logs,prompts,runs}` worker artifacts; legacy `generated_files` entries in `.refactor-loop/state/runtime-retention-plan.json` are ignored and grant no delete authority. The helper targets same-inode compaction of `$REPO_ROOT/.refactor-loop/.controller-pending-events.log` and `$REPO_ROOT/.refactor-loop/.concurrency-alert.log`, preserves/consumes planner-proven stale `$REPO_ROOT/.worktrees/<name>` entries after local git read rechecks, and unlinks only old regular spawn-task lock files whose safe basename, metadata, repo-local log path, and matching log `EXIT=` marker all validate. It may run only `git worktree remove <path>` and `git worktree prune`; it must not create archive/index state, call GitHub, run `git fetch`, delete branches, delete worker logs/prompts/runs, commit, push, reset, rebase, merge, tag, release, spawn codex, become a daemon, add `RuntimeRetentionPlan.spawn_task_locks`, use missing-log companion/plan proof, or expose a compatibility alias. Verification lives in `test_runtime_retention.py`.
...
Required fields are `cd`, `prompt`, and `log`; `task_id` defaults to the `.dispatch.json` filename stem if omitted, and `stall` defaults to `5400` if omitted. Dispatch queue payload paths remain absolute at the queue boundary so floor counting can still scope by `$REPO_ROOT`; emitted `HARNESS_SPAWN_INTENT` paths use the canonical repo-relative writer form and are checked by the shared consumer validator.
...
- On each tick, if `actual < CODEX_FLOOR` and the queue is non-empty, the monitor queues at most `CODEX_FLOOR - actual` `HARNESS_SPAWN_INTENT` events for #396 wakeup-runner consumption.
...
- The monitor writes `HARNESS_SPAWN_INTENT` plus `DISPATCH_INTENT:<task-id>:<priority>:<reason>` to `.refactor-loop/.controller-pending-events.log`; archive means runner-visible intent queued, not worker process launched.
...
| IMPLEMENT_DONE:ok | controller commit + helper-private publish verification + safe-push; `publish_implementation_output` opens or updates exactly one managed implementation PR + Consensus-rnd Phase review-gate reviewer × 3 |
...
**正确语义**:常规 `HARNESS_SPAWN_INTENT` 的执行 owner 是 #396 `wakeup-runner`: deterministic daemon 写 prompt/log/ledger 或 intent,runner 通过 checked-in spawn helper 启动 codex。controller/harness 直接 Bash background task 只用于 [Daemon-first controller boundary](#daemon-first-controller-boundary) 中已记录的 mechanical daemon-stuck fallback。
...
0. **host.env 自检(缺失即停,绝不臆造)**:`test -n "${CONSENSUS_RND_HOST_ENV:-}" && source "$CONSENSUS_RND_HOST_ENV"` 取 `$REPO_ROOT/$GH_REPO_SLUG/$BUILD_CMD/$TEST_CMD/...`。
   - 不存在 → 从 `skills/consensus-loop/host.env.example` 复制到 host-owned `.config/consensus-rnd/host.env` 并设置 `CONSENSUS_RND_HOST_ENV` 并填必填项;无法确定必填值(REPO_ROOT/GH_REPO_SLUG/BUILD_CMD/TEST_CMD)→ **PushNotification 请 maintainer 填,end turn,不 spawn 任何东西**。
...
0b. **ProjectRulesFixedPointProbe(强制,先于任何 actor 派发)**:在 `host.env` 注入后立即运行:
   ```bash
   python3 skills/consensus-loop/scripts/consensus-rnd-cli check-project-rules
   ```
...
6b. **Open PR** (**body follows `$HOST_WORK_LANGUAGE`; no mandatory parallel English section**):
    ...
    🤖 Auto-loop / consensus-loop iter<N>
    ```
...
3. `CHECK_REBASE`: if a rebase is in progress with unresolved paths, reuse the same resolver worker contract. If all paths are resolved, only a prior `adopt-merged-rollup` artifact with matching expected remote SHA may produce `continue-resolved-rollup-adoption-rebase`; the executor runs `git rebase --continue`, verifies replay completeness, and then force-with-lease pushes. Any replay-integrity mismatch fails closed before push.
4. `CHECK_DIRTY`: dirty non-merge/non-rebase worktrees skip without reset.
5. `LOCAL_AHEAD_PENDING_ONLY`: compute `local_ahead_count` with `git rev-list --count origin/$INTEGRATION_BRANCH..HEAD`; if the daemon worktree is clean and ahead, append `DEV_SYNC_PENDING:local-ahead-managed-adoption-required:<json>` and stop the tick. Local-ahead/diverged checkout state is not a publish source and must go through managed adoption PR/review recovery.
6. `ADOPT_MERGED_ROLLUP`: if a merged rollup PR from `$INTEGRATION_BRANCH` to `$REVIEW_BASE_BRANCH` is provable by `headRefName == $INTEGRATION_BRANCH`, `headRefName == rollup/<expected origin/$INTEGRATION_BRANCH sha>`, or `headRefOid == expected origin/$INTEGRATION_BRANCH sha`, capture the old rollup head and current expected remote SHA, then write and execute an `adopt-merged-rollup` operation. Noncurrent or foreign `rollup/*` merged heads are ignored; the executor re-checks ancestry and live SHAs before force-with-lease adoption.
7. `RESET_TO_REMOTE`: after rollup adoption checks, write and execute a `reset-to-remote` operation for remote alignment when local HEAD differs from `origin/$INTEGRATION_BRANCH` and there is no local-ahead state.
8. `FORWARD_SYNC`: when review base needs to be incorporated into integration, write and execute a `forward-sync-review-base` operation. The executor re-checks live state before merge and push.
9. `DETECT_RELEASE_ROLLUP_NEEDED`: if `origin/$INTEGRATION_BRANCH` is ahead of `origin/$REVIEW_BASE_BRANCH` by at least `RELEASE_ROLLUP_MIN_COMMITS` and no open rollup PR already covers that integration SHA, append `DEV_SYNC_PENDING:release-rollup-needed:<json>` with branch names, SHAs, ahead count, timestamp, and reason. Cooldown only suppresses duplicate same-SHA events; it grants no lifecycle authority.
10. `MISSING_INTEGRATION_BRANCH_ALERT`: daemon startup/tick verifies `origin/$INTEGRATION_BRANCH` exists via `git ls-remote --exit-code --heads origin "$INTEGRATION_BRANCH"`. If missing, append `DEV_SYNC_PENDING:missing-integration-branch:<branch>` and stop the tick without guessing or recreating the branch.
...
- Restart-helper-managed daemon commands are defined by `restart.py::DAEMON_COMMANDS` and projected by `restart_managed_daemon_names()`; restart-local `DaemonRuntimePolicy` mirrors only that target cadence and is not a daemon registry, lifecycle authority, host production SSOT, or #191 lease substitute.
...
- DesignConsensusIssueIntake: open `crnd:lifecycle:managed` + `crnd:phase:design-solving` issue, source-OPEN gate passed → queues each r1 solver role (`minimal`, `structural`, `delete`) whose role-specific ledger key, r1 evidence/log, and validated pending target intent are absent as that role's r1 `HARNESS_SPAWN_INTENT` with router-rendered prompt/log/output paths and manual issue provenance; existing evidence/log/pending intent for one solver role suppresses only that role.
- `SOLVER_DONE:<minimal|structural|delete>:*` x3, same issue/round, clean `^EXIT=0`, non-placeholder, not ledgered, and no target/equivalent log or validated pending target intent → render full `prompts/meta-judge.md` with router-scoped inputs and queue same-round meta-judge `HARNESS_SPAWN_INTENT`.
- `META_JUDGE_DONE:converge:round-<N>:*`, clean exit, not ledgered and no target/equivalent log or validated pending target intent → clean rS judge canonical payload is `round-S`; legacy `round-(S+1)` is temporarily accepted; before queueing r(S+1) minimal/structural/delete solver intents, run the router-owned stalled predicate(`round >= 3` and matching router-normalized no-progress signatures across 3 rounds); if it holds, queue round-S reflector intent with the full `prompts/meta-reflector-stalled.md` template and suppress next solvers; non-adjacent payload mismatch falls back.
...
- `META_RESOLVED:re-design` from reflector to source-adjacent `marker.round + 1` solver triplet: clean exit, source-OPEN gate passed, not ledgered, no target/equivalent log or validated pending target intent, terminal gate open → queue minimal/structural/delete solver `HARNESS_SPAWN_INTENT`; collisions are suppressed by ledger/log/pending-intent/terminal/source-open gates, not by skipping to a later round.
...
Fallback/ledger/recovery: lifecycle/unknown markers append `.refactor-loop/.controller-pending-events.log`; no direct codex process spawn, no direct resolution, no git, no GitHub lifecycle mutation, no label, no lifecycle authority(no close/merge/release). Direct routes append `HARNESS_SPAWN_INTENT`; its `command` field is exactly `"spawn-codex"` as a closed semantic enum, not argv and not shell. The mirror token is `command: "spawn-codex"`; forbidden boundary includes daemon direct `nohup spawn-codex`. `argv`, `args`, `shell`, `cmd`, `commands`, `env`, `git`, `gh`, `executor`, `target_ref`, argv array, shell command, and generic command bus fields are forbidden; the #396 wakeup-runner consumes valid intent through the checked-in spawn helper, where CLI argv construction stays outside the intent protocol. Controller/harness direct spawn is only a mechanical daemon-stuck fallback. Append-only `.refactor-loop/phase9-router-ledger.jsonl` records base dispatch fields `{key, marker, log_path, dispatched_at, dispatch_state}` where `dispatch_state="harness-intent"` means intent queued, not process launched; successful solver-triplet-to-judge rows may add row-level router-private provenance fields `{route, issue, round, target_actor, clean_exit_solver_logs, solver_input_prompts, judge_input_solver_logs, judge_prompt_path, judge_prompt_template_path, judge_prompt_scope, independence_check}`. Router recovery/idempotency reads only `key`; the detailed recovery pass is a router-private `Phase9ActorHealth` projection over structured log/marker/path/ledger/pending-intent/source/terminal facts. meta-judge decisions read solver logs, not ledger evidence. Clean markerless solver logs are preserved in place, then the router appends one existing-format fallback event with reason `phase9-solver-markerless-exhausted`; this evidence is terminal format failure and may not later reuse `actor_health_recovery`. The stale ledger bypass for truly missing/interrupted actors is conjunctive: the source issue is OPEN, the terminal gate is open, there is no valid actor marker, no target log, no equivalent legacy log, no pending `HARNESS_SPAWN_INTENT`, and the latest append-only ledger row is older than `STALE_REVIVAL_HOURS` (default 3h). The router does not read the process table or spawn-task locks as dispatch predicates; same-device duplicate execution after a valid intent is handled only by `TaskSpawnClaimStore.acquire(...)` at the `spawn-codex` boundary using the same log-derived task id held-claim skip/noop. If router-visible solver prompts explicitly reference same-round peer solver logs/prompts/run artifacts, the router fails closed before judge dispatch and appends an existing-format pending event with reason `phase9-triplet-evidence-invalid`; if full `prompts/meta-judge.md` rendering is unavailable or any solver/judge path falls outside same issue/round/role scope, the router fails closed before judge dispatch and appends an existing-format `phase9-router-fallback` event. Fallback events use prefix `phase9-router-fallback`. A solver-triplet-to-judge duplicate with `key` already in the ledger is silent; when the triplet is not ledgered but target log / equivalent legacy judge log / validated pending target intent suppresses dispatch, append one existing-format `phase9-router-fallback` event with key prefix `phase9-triplet-suppression:` and reason exactly one of `phase9-triplet-target-log-exists`, `phase9-triplet-equivalent-log-exists`, or `phase9-triplet-pending-intent`. Terminal design-consensus suppression appends one existing-format `phase9-router-fallback` event with key prefix `phase9-terminal-eligibility:` and reason exactly `phase9-already-consensus`; restart recovery seeds `phase9-terminal-eligibility:` and `phase9-actor-health:` keys from pending events to keep duplicate fallback emission suppressed, including markerless exhausted events. The source-OPEN gate and issue source snapshot reads, state-only source-OPEN gate mirror token, and labels-only terminal gate must not use gh issue close, gh issue edit, gh label, gh pr merge, gh release, or any label/close/merge/release lifecycle flag. Target/equivalent logs or validated pending `HARNESS_SPAWN_INTENT` suppress re-dispatch, `.refactor-loop/phase9-router.lock` enforces singleton, and duplicate ledger rows never delete logs. This is no public revive command, no new runtime exception, and no new lifecycle authority. Staged expansion requires route-ledger evidence and must not introduce ControllerEvent, ControllerCommand, SpawnIntentInbox, spawn-intents, ControllerOrchestrator, migrated work-unit schema, public marker aliases, or lifecycle authority.
### Daemon vs controller 分工
dev sync stays with daemon; Consensus-rnd Phase design-consensus issue intake, triplet/converge/router-derived stalled continuation, legacy stalled compatibility, and reflector `META_RESOLVED:re-design` continuation may use **consensus-rnd-cli phase9-router** narrow allowlist; wakeup-plan design-consensus completed-marker evidence is status-only and suppresses design-consensus solver `HARNESS_SPAWN_INTENT` actions for terminal phase labels or exactly-one open managed closing PR evidence. consensus/implement/review/fix/escalation routes default to #396 helpers / daemon-owned closed action projection, with interactive controller takeover only after recorded mechanical daemon-stuck evidence in [Daemon-first controller boundary](#daemon-first-controller-boundary). liveness duty is limited to `daemon-status` + `restart-daemons` + status re-read; controller fallback sweep is diagnostic rather than authoritative dispatch.
...
After a rollup PR has merged into `review_base_branch`, `integration sync daemon` must make `integration_branch` contain that merged review-base head before new forward sync work. Any post-rollup integration commits are replayed only after the proven old rollup head; if the old head or expected remote SHA cannot be proven, the daemon writes a pending event and does not force-push. If the daemon-owned adoption rebase pauses for conflicts, continuation is only `continue-resolved-rollup-adoption-rebase` bound to the original adoption artifact; replay-integrity mismatch fails closed before push.
...
Each reviewer outputs `REVIEW_DONE:${PR}:${role}:<approve|comment|reject>` in its local artifact/log and posts a GitHub-visible PR comment with final AI sentinel, `review_round`, `head_sha`, and the same marker. `wakeup-runner` computes `FIX`, `MERGE`, and `MERGE_WITH_COMMENTS` only from GitHub-visible, final-sentinel, same-head reviewer comments for the live PR head. Local `.refactor-loop/runs/review-pr<N>-<role>-r<R>.md` and `.refactor-loop/logs/review-pr<N>-<role>-r<R>.log` files remain diagnostics, prompt inputs, completion/pending hints, and stale redispatch hints only; they are not merge/fix authority.
...
|---|---|
| Target-required checks green, PR mergeable, every required role has exactly one GitHub-visible final-sentinel reviewer comment for the same review round, and every required reviewer comment head SHA equals the live PR head | `reject=0`, `approve=R`, `comment=0` | `MERGE`: post merge comment according to `$HOST_WORK_LANGUAGE`, then call `merge_pr <pr>` for ready+merge. |
...
| Any gate incomplete or invalid | missing role, duplicate/unknown verdict, missing final AI sentinel, missing review round, missing/stale per-reviewer head SHA, target-required CI pending/fail/missing/unavailable, or non-mergeable PR | `WAIT_OR_REDISPATCH`: wait or re-dispatch invalid/missing reviewer through bounded `review_evidence_recovery.py` keys; `CONFLICTING`/`DIRTY` is status-only; do not ready, never merge. |

Same-head no-progress protection is part of the same review recovery owner surface. If GitHub-visible final-sentinel comments show the same complete same-head verdict signature in multiple rounds, `review_evidence_recovery.py` emits status-only `repeated_review_blocker` for repeated reject signatures, or an explicit-approval wait for repeated all-comment/no-approval signatures; it must not dispatch another review-fix or reviewer prompt from local round history. Old-head signatures and cross-head same-PR round counts are diagnostic only and never suppress fresh-head reviewer opportunity.

Clean worker terminal marker consumers use `codex_refactor_loop.worker_markers` as the shared source for local completion detection, implement readiness, prompt inputs, and redispatch hints. The reader accepts standalone allowlisted terminal markers only after clean `EXIT=0`; solver completion/conflict evidence comes only from raw column-zero `SOLVER_DONE:<role>:` emissions, while diff/prose/quoted echoes are context only. When the log lacks a trusted raw marker it may read only the same-stem `.refactor-loop/runs/<stem>.md` companion artifact for allowlisted worker roles. Duplicate, malformed, or conflicting local marker evidence fails closed for those local completion surfaces, including raw solver wrong-role, malformed, or conflicting markers. Implement readiness recognizes `IMPLEMENT_DONE:*:ok` through this log-first plus same-stem companion artifact path. Review-gate merge/fix authority is GitHub-comment-first: local review artifacts and logs do not authorize `FIX`, `MERGE`, or `MERGE_WITH_COMMENTS`.
...
If PR is pushed after consensus (rebase, requested change), head SHA changes. Next Consensus-rnd Phase review-gate sweep requires every required GitHub-visible reviewer comment head SHA to match the current head SHA; any missing or stale per-reviewer comment drops prior consensus and re-dispatches all reviewers. Local review artifact verdicts do not bypass current-head binding; merge readiness requires every required reviewer comment to bind to the live PR head. Never auto-merge stale consensus.
...
The command shapes below describe the equivalent checked-in spawn helper invocation produced by daemon-owned dispatch surfaces: `phase9-router` writes `HARNESS_SPAWN_INTENT`, `wakeup-runner` consumes the closed action projection, and controller manual execution of these shapes is only the stuck-gated fallback in [Daemon-first controller boundary](#daemon-first-controller-boundary).
...
**规则**:常规 `HARNESS_SPAWN_INTENT` 由 #396 `wakeup-runner` 消费并调用 checked-in spawn helper;stuck-gated controller fallback 的 codex spawn 才用 Bash tool `run_in_background: true`。Claude Code harness 跟踪 fallback background task,显示在 UI shells/tasks 面板 → maintainer 看到 "8 shells" 等计数。`nohup ... & disown` 会 detach 出 harness,maintainer 看不到即时 shells/task-notification;若意外发生,不要杀掉重派,必须确认 log 可 sweep 且本 turn 结束前有已注册 ScheduleWakeup 或其它在飞 task-notification。
...
Naming policy: this engine's public product identity is Consensus R&D, and `consensus-loop`
...
- `actual`: current this-loop `consensus-rnd-cli spawn-codex` process count from the observation-only bounded/fail-open concurrency reader; this is status and floor telemetry, not phase9-router dispatch authority.
...
- Treat the marker as non-publishable retry evidence, not a terminal implement result.
- Fresh non-ok markers wait for `$STALE_REVIVAL_HOURS`; stale markers with no canonical in-flight worker may be cleared and redispatched through existing `dispatch_consensus_implementation`.
- If repeated retries show scope ambiguity, broken tests, wrong design, or impossible requirements, feed the blocker back through the existing prompt/design-consensus route instead of publishing or applying lifecycle effects from the non-ok marker.
