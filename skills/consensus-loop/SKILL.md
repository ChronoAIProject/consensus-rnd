...
Narrow allowlist: only when `$RUNTIME_RETENTION_ENABLE=true`, RuntimeRetention produces the owner-local `generated_files` slice by scanning only `$REPO_ROOT/.refactor-loop/{logs,prompts,runs}` for old generated regular non-symlink files, atomically merges that slice into `.refactor-loop/state/runtime-retention-plan.json`, then consumes that same plan as the sole planner proof. Additive `generated_files` entries may delete only TTL-expired repo-relative regular non-symlink files under `$REPO_ROOT/.refactor-loop/{logs,prompts,runs}` after executor revalidation proves planner eligibility, `generated_file`, `ttl_expired`, `no_in_flight`, `no_open_actionable`, `no_pending_intent`, `no_unconsumed_marker`, and `no_recovery_surface`; missing, malformed, ambiguous, young, open-target, in-flight, symlink/FIFO, path-escape, pending-reference, markerless recovery, or unproven files are kept fail-closed and counted in `kept`. It also compacts `$REPO_ROOT/.refactor-loop/.controller-pending-events.log` in place so Monitor bridges keep the same inode; and preserves/consumes stale `$REPO_ROOT/.worktrees/<name>` entries present in the same plan with proof fields `no_in_flight`, `no_open_issue_or_pr`, `no_dirty`, `no_local_ahead`, and `merged_or_missing_safe`, after local git read rechecks pass. It may then run `git worktree remove <path>` and `git worktree prune`. Missing/false opt-in exits 0 with a noop summary.
...
- [scripts/consensus-rnd-cli runtime-retention](scripts/consensus-rnd-cli runtime-retention) — canonical RuntimeRetention helper for host-opt-in planner-proven generated-file deletion, pending-events compaction, and planner-proven stale worktree remove/prune.
...
`consensus-rnd-cli restart-daemons` also runs daemonless RuntimeRetention before daemon freshness checks. `consensus-rnd-cli runtime-retention` has no GitHub lifecycle authority and is disabled unless `$RUNTIME_RETENTION_ENABLE=true`. When enabled, RuntimeRetention produces the owner-local `generated_files` plan slice from `$REPO_ROOT/.refactor-loop/{logs,prompts,runs}`, atomically merges it into `.refactor-loop/state/runtime-retention-plan.json`, then consumes that plan: additive `generated_files` entries may delete only planner-proven, TTL-expired, repo-relative regular non-symlink files under `$REPO_ROOT/.refactor-loop/{logs,prompts,runs}` after local executor revalidation keeps young, open/in-flight, pending-reference, unconsumed-marker, markerless recovery, symlink/FIFO, path-escape, malformed, and unproven files fail-closed; the same helper still targets same-inode compaction of `$REPO_ROOT/.refactor-loop/.controller-pending-events.log` and preserves/consumes planner-proven stale `$REPO_ROOT/.worktrees/<name>` entries after local git read rechecks. It may run only `git worktree remove <path>` and `git worktree prune`; it must not create archive/index state, call GitHub, run `git fetch`, delete branches, commit, push, reset, rebase, merge, tag, release, spawn codex, become a daemon, or expose a compatibility alias. Verification lives in `test_runtime_retention.py`.
