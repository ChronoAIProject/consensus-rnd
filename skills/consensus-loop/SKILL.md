...
- **Narrow allowlist**: helper owns singleton wrappers + actor-owned heartbeat leases + helper-private launch fingerprints for `concurrency_monitor`, `comment-monitor`, `codex-progress-reporter`, `dev_sync_daemon`, `phase9_router_daemon`, `closed_label_reconciler` in the existing static daemon allowlist;each wrapper may self-heal only its own child on child exit or missing/malformed/stale actor heartbeat by reading the actor-owned heartbeat and restarting the same resolved command;heartbeat 是 actor-loop progress lease,不是 wrapper sidecar liveness;daemon actor after tick / caught exception / lease sleep renews it;fingerprint artifact `.refactor-loop/locks/<daemon>.fingerprint.json` 只记录 daemon name、resolved command、CLI entrypoint hash、Python package tree hash、文件计数、`host_env_path` 和 `host_env_sha256`,只用于 restart skip eligibility and read-only stale projection;it never stores host.env plaintext and never becomes host production SSOT, branch topology, durable ledger, host artifact authority, or side-effect authorization;`DaemonProcessInventory` owns the restart-helper-local `DaemonInstanceProjection` and only matches same repo_root / daemon target / known command shape facts: canonical restart wrappers, managed child command instances, and bounded lock-holder evidence from target-owned local lock files. missing/malformed/mismatch fail-closed,发现 duplicate canonical live wrapper 或 orphan bounded lock-holder child 时 fail closed:启动或修复前 terminate 全部同仓库同 daemon 旧 wrapper 和 projection-proven bounded lock-holder child,随后只 spawn 一个 fresh wrapper;the controller's periodic wakeup is the primary supervisor that invokes restart-daemons, and cron/launchd is an optional outer keepalive only for unattended operation with no controller session covering wrapper death, boot/post-wake, fingerprint/host.env reload, duplicate cleanup, bounded lock-holder cleanup, and #191 owner refresh;并顺手运行 canonical `consensus-rnd-cli runtime-retention`;不 spawn codex / commit / push / merge / label / archive。
- **Read-only status projection**: `consensus-rnd-cli daemon-status --json` mirrors the same static allowlist and helper-private pid/heartbeat/fingerprint/inventory facts plus cached active-controller status, and projects duplicate count, managed child command pids, and bounded lock-holder pids from the same `DaemonInstanceProjection`. Fresh heartbeat plus no wrapper plus orphan bounded lock-holder child is `stale` with grepable `orphan-lock-holders:N` diagnostics, not `running`. It is read-only status only, has no public start/stop/restart/reload lifecycle verb, and repair/reload remains restart-daemons.
