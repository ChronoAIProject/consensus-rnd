...
| `IMPLEMENT_DONE:ok` | The only publishable implement marker. Controller verifies worker-authored `.refactor-loop/runs/implementation-pr-${CLUSTER_ID}-title.txt` and `.refactor-loop/runs/implementation-pr-${CLUSTER_ID}-body.md`; after real scoped diff validation, commit, and integration-base restore, `publish_implementation_output` queues or harvests a `publish_verification.py` job under `.refactor-loop/state/publish-verification/jobs/<job_key>/`. Parent ticks write immutable `request.json`, pin `refs/consensus/publish/<job_key>`, start at most one hidden one-shot `wakeup-runner --run-one-publish-ratchet <job_dir>` child when a slot is free, and return without waiting. Only a revalidated `VERIFIED` `result.json` receipt finalizes by pushing the literal verified object id with `git push origin <candidate_sha>:refs/heads/<head_ref>`, confirming remote OID equality, then updating exactly one matching open managed implementation PR or opening and verifying exactly one managed implementation PR when zero matching PRs exist, then confirming Consensus-rnd Phase review-gate reviewer dispatch evidence. If the exactly matching open managed PR already points at canonical worktree `HEAD`, `wakeup-plan` emits `suppressed_reason=pr_already_open_current` status-only and the helper no-ops before heavy work; Remote ref proof is read-only and bounded. Missing or invalid PR artifacts, duplicate/multiple PRs, conflict PRs, head/base/link mismatch, unmanaged PRs, failed or mismatched publish verification, unknown/not-current PR head evidence, stale-base clean output, superseded jobs, or quarantined retry state fail closed/status-only or fall through to the existing publish path. |
...
5. `IMPLEMENT_DONE:ok` means the controller may inspect diff, commit, restore integration base, queue or harvest the helper-private `publish_verification.py` job/receipt, finalize only a revalidated `VERIFIED` receipt by literal-SHA push plus remote OID equality, then have `publish_implementation_output` open or update exactly one managed implementation PR; duplicate/multiple/mismatch/unmanaged PRs fail closed.
...
| IMPLEMENT_DONE:ok | controller commit + `publish_verification.py` job/receipt + literal-SHA finalizer; `publish_implementation_output` opens or updates exactly one managed implementation PR + Consensus-rnd Phase review-gate reviewer × 3 |
...
3. **Publish implementation branch** through `publish_implementation_output`:
   the helper queues or harvests the `publish_verification.py` job/receipt, finalizes only a revalidated `VERIFIED` receipt with `git push origin <candidate_sha>:refs/heads/<head_ref>`, and confirms remote OID equality before any PR open/update.
...
Each reviewer outputs `REVIEW_DONE:${PR}:${role}:<approve|comment|reject>` in its local artifact/log and posts a GitHub-visible PR comment with final AI sentinel, `review_round`, `head_sha`, and the same marker. `wakeup-runner` computes `FIX`, `MERGE`, and `MERGE_WITH_COMMENTS` only from GitHub-visible, final-sentinel, same-head reviewer comments for the live PR head. Local `.refactor-loop/runs/review-pr<N>-<role>-r<R>.md` and `.refactor-loop/logs/review-pr<N>-<role>-r<R>.log` files are diagnostics, prompt inputs, completion/pending hints, and stale redispatch hints only; they are not merge/fix authority. Supervisor-refreshed log mtime and matching same-device spawn holder PID feed `reviewer_liveness.py` pending/terminal/redispatchable facts only.
