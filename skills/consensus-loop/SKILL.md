...
=======
| `IMPLEMENT_DONE:ok` | The only publishable implement marker. Controller verifies worker-authored `.refactor-loop/runs/implementation-pr-${CLUSTER_ID}-title.txt` and `.refactor-loop/runs/implementation-pr-${CLUSTER_ID}-body.md`; after real scoped diff validation, host checks, commit, integration-base restore, and safe-push of the canonical head, `publish_implementation_output` updates exactly one matching open managed implementation PR or opens and verifies exactly one managed implementation PR when zero matching PRs exist, then confirms Consensus-rnd Phase review-gate reviewer dispatch evidence. If the exactly matching open managed PR already points at canonical worktree `HEAD`, `wakeup-plan` emits `suppressed_reason=pr_already_open_current` status-only and the helper no-ops before heavy work; Remote ref proof is read-only and bounded. Missing or invalid PR artifacts, duplicate/multiple PRs, conflict PRs, head/base/link mismatch, unmanaged PRs, unknown/not-current PR head evidence, or stale-base clean output fail closed/status-only or fall through to the existing publish path. |
