...
PACKAGE_ROUTER = REPO_ROOT / "skills" / "consensus-loop" / "scripts" / "codex_refactor_loop" / "phase9" / "router.py"
...
class Phase9RouterPackageTests(unittest.TestCase):
    ...
    def test_package_router_stalled_normalizes_unreachable_source_oscillation(self) -> None:
        verdicts = {
            1: "escalate:no-plan:source-location-missing-or-invalid",
            2: "abstain:no-current-source-target",
            3: "propose:source-unreadable-current-checkout",
        }
        for round_no, verdict in verdicts.items():
            for role in ("minimal", "structural", "delete"):
                self.write_log(f"phase9-issue708-r{round_no}-{role}.log", f"SOLVER_DONE:{role}:{verdict}:summary")
    ...
    def test_package_router_implementation_bearing_propose_blocks_stalled(self) -> None:
        ...
        for round_no, verdict in verdicts.items():
            for role in ("minimal", "structural", "delete"):
                self.write_log(f"phase9-issue709-r{round_no}-{role}.log", f"SOLVER_DONE:{role}:{verdict}:summary")
    ...
    def test_package_router_source_uses_meta_judge_template_not_stub_prompt(self) -> None:
        src = PACKAGE_ROUTER.read_text(encoding="utf-8")
        skill = (REPO_ROOT / "skills" / "consensus-loop" / "SKILL.md").read_text(encoding="utf-8")
        combined = "\n".join((src, skill))
        for required in (
            "prompts/meta-judge.md",
            "MetaJudgePromptRenderer",
