...
def copy_minimal_repo() -> tempfile.TemporaryDirectory[str]:
    ...
    paths = [
        ...
        "skills/consensus-loop/SKILL.md",
        "skills/consensus-loop/host.env.example",
        "skills/consensus-loop/scripts/consensus-rnd-cli",
        "skills/consensus-loop/scripts/codex_refactor_loop/release/gate.py",
        "skills/consensus-loop/scripts/codex_refactor_loop/release/required_checks.py",
        "skills/consensus-loop/scripts/codex_refactor_loop/checks/degradation.py",
        "skills/consensus-loop/scripts/codex_refactor_loop/checks/host_fixture_smoke.py",
        "skills/consensus-loop/scripts/codex_refactor_loop/monitors/concurrency.py",
        "skills/consensus-loop/scripts/codex_refactor_loop/peek.py",
    ]
    ...
    shutil.copytree(
        REPO_ROOT / "skills/consensus-loop/scripts/codex_refactor_loop",
        repo / "skills/consensus-loop/scripts/codex_refactor_loop",
        dirs_exist_ok=True,
...
class SkillDegradationCheckerBehaviorTests(unittest.TestCase):
    ...
    def test_static_checker_treats_plugin_host_root_as_not_source_repo(self) -> None:
        ...
        self.assertNotIn("required-file: skills/consensus-loop", result.stdout)
    ...
    def test_static_checker_fails_closed_for_damaged_source_repo_candidate(self) -> None:
        ...
        self.assertIn("required-file: skills/consensus-loop", result.stdout)
    ...
    def test_static_checker_does_not_require_reference_md(self) -> None:
        with copy_minimal_repo() as tmp:
            repo = Path(tmp) / "repo"
            reference = repo / "skills/consensus-loop/REFERENCE.md"
            if reference.exists():
                reference.unlink()
    ...
    def test_checker_detects_missing_source_repo_validation_contract(self) -> None:
        with copy_minimal_repo() as tmp:
            repo = Path(tmp) / "repo"
            skill = repo / "skills/consensus-loop/SKILL.md"
            skill.write_text(
    ...
    def test_checker_detects_forbidden_runtime_file(self) -> None:
        with copy_minimal_repo() as tmp:
            repo = Path(tmp) / "repo"
            forbidden = repo / "skills/consensus-loop/scripts/degradation_watchdog.py"
    ...
    def test_checker_detects_release_projection_missing_host_required_checks_parser(self) -> None:
        with copy_minimal_repo() as tmp:
            repo = Path(tmp) / "repo"
            projection = repo / "skills/consensus-loop/scripts/codex_refactor_loop/release/required_checks.py"
    ...
    def test_checker_detects_downstream_runtime_watch_marker_drift(self) -> None:
        with copy_minimal_repo() as tmp:
            repo = Path(tmp) / "repo"
            monitor = repo / "skills/consensus-loop/scripts/codex_refactor_loop/monitors/concurrency.py"
