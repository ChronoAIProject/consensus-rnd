...
class AntiStopRestartHelperContractTests(unittest.TestCase):
    ...
    def test_runtime_exception_mirror_mentions_launch_fingerprint_contract(self) -> None:
        for needle in (
            ...
            "requires host-owned `CONSENSUS_RND_HOST_ENV` base admission before `_prepare_dirs()`",
            "direct `start_daemon()`",
            "`MAINTAINER_WHITELIST` is not a fleet-wide restart gate",
            "`DAEMON_STARTUP_FAIL_CLOSED daemon=comment-monitor reason=<field>-unset`",
