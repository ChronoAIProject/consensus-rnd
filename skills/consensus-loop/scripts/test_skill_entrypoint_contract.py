#!/usr/bin/env python3
"""Source contract tests for the consensus-loop single-file contract/reference skill."""
...
AUTH_MIRROR = "skills/consensus-loop/authorizations/runtime-exceptions.md"
...
class SkillEntrypointContractTests(unittest.TestCase):
    ...
    def test_frontmatter_contract_is_minimal_and_trigger_only(self) -> None:
        ...
        self.assertEqual(lines[0], "name: consensus-loop")
    ...
    def test_worker_terminal_marker_reader_contract_is_local_fact_source(self) -> None:
        ...
        for needle in (
            ...
            "solver completion/conflict evidence comes only from raw column-zero `SOLVER_DONE:<role>:` emissions",
            "diff/prose/quoted echoes are context only",
            ...
            "raw solver wrong-role, malformed, or conflicting markers",
