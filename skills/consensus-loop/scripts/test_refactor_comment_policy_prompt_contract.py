...
class RefactorCommentPolicyPromptContractTests(unittest.TestCase):
    ...
    def test_default_policy_is_none_and_external_rationale(self) -> None:
        ...
        self.assertIn("Comments on tiny helper types must also follow `${HOST_REFACTOR_COMMENT_POLICY}`", implement)
        self.assertIn("do not write `refactor helper`, `no behavior change`, `Old`, `New`, or `iterN`", implement)
        self.assertIn("write accurate English business-behavior prose only", implement)
        self.assertIn("Use the existing Refactor self-documentation format from item 2 only when explicit `self-doc-comment` is set", implement)
    ...
    def test_explicit_self_doc_policy_preserves_old_new_requirement(self) -> None:
        ...
        self.assertIn("`self-doc-comment`: each refactored class/key method must", implement)
        ...
        self.assertIn("Missing any required comment without a reasonable not-applicable explanation is a defect", verify)
    ...
    def test_none_policy_forbids_refactor_history_source_comments(self) -> None:
        ...
        verify = read_prompt("verify.md")
        self.assertIn("Newly added `Refactor (...)`, `Old pattern`, `New principle`, or `iterN/cluster` refactor-history source comments are defects", verify)
