...
class SkillReferenceAnchorTests(unittest.TestCase):
    ...
    def test_issue_579_consensus_gate_proof_anchor_and_boundaries(self) -> None:
        ...
        for needle in (
            ...
            "skills/consensus-loop/authorizations/runtime-exceptions.md#consensus-gate-proof-579",
    ...
    def test_skill_documents_update_check_notify_only_contract(self) -> None:
        ...
        for needle in (
            ...
            "test_statusline.py",
            "skills/consensus-loop/authorizations/runtime-exceptions.md#update-check-231",
        ):
            with self.subTest(needle=needle):
                self.assertIn(needle, section)
    ...
    def test_skill_documents_controller_release_publisher_exact_sha_gate(self) -> None:
        ...
        for needle in (
            "skills/consensus-loop/authorizations/runtime-exceptions.md#controller-release-publisher-334",
            ...
            "only then `gh release create v<to_version> --target <fresh release commit sha> --notes-file <controller-generated release notes file>",
            "test_release_notes.py",
            "Missing `GH_REPO_SLUG`",
            "pending/red/missing/stale exact-SHA required checks",
            "invalid Checks API JSON",
            "Checks API failure",
            "fail closed before release notes generation",
            "the notes file is a controller-private artifact generated after exact-SHA checks and before release creation",
    ...
    def test_harness_spawn_intent_contract_documents_shared_consumer_validator_boundary(self) -> None:
        ...
        self.assertIn("skills/consensus-loop/authorizations/runtime-exceptions.md#phase9-router-open-state-gate-229", self.skill)
...
class WakeupRunnerContractTests(unittest.TestCase):
    ...
    def test_consensus_implementation_projection_fact_source_is_judge_only(self) -> None:
        ...
        for needle in (
            ...
            "transitions labels to `crnd:phase:blocked` plus `crnd:human:auto`",
            "non-empty `scope_paths` consensus continues to `dispatch_consensus_implementation`",
            "`suppressed_reason`",
            "`open_closing_pr`",
            "`remote_iter_branch`",
            "`in_flight_implement`",
    ...
    def test_implementation_publish_runbook_forbids_legacy_early_pr_contract_tokens(self) -> None:
        ...
        combined = "\n".join(sections)
        for required in (
            "`publish_implementation_output` queues or harvests a `publish_verification.py` job",
            "immutable `request.json`",
            "`refs/consensus/publish/<job_key>`",
            "hidden one-shot `wakeup-runner --run-one-publish-ratchet <job_dir>` child",
    ...
    def test_controller_tick_supervisor_anchor_documents_key_only_no_lifecycle_contract(self) -> None:
        ...
        for needle in (
            ...
            "First migrated target: `comment-monitor`",
            "run_comment_monitor_reconcile_tick()",
            "CommentMonitor.tick()",
            "managed_work_snapshot",
            "phase9-router` and `dev-sync` are not first migration targets",
