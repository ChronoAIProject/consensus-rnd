...
ROLE_NEXT_STEPS = {
    "en": {
        "test-add": "1. test-add emits marker `TEST_ADD_DONE:...`  2. controller commits + pushes  3. codecov reruns",
        "fix": "1. fix r<N> emits marker `FIX_DONE:...`  2. controller commits + pushes  3. dispatches reviewer r<N+1>",
        "reviewer": "1. three reviewers emit verdict markers  2. controller computes consensus  3. reject=0 + approve>=1 -> merge; all-comment -> wait explicit approval; reject -> fix",
        "implement": "1. implement emits marker `IMPLEMENT_DONE:<cluster>:<status>`  2. controller commits + pushes  3. opens PR + dispatches reviewer r1",
        "solver": "1. three solvers emit `SOLVER_DONE:...`  2. controller dispatches meta-judge r<N>  3. consensus -> implement / converge -> fresh round",
        "judge": "1. judge emits `META_JUDGE_DONE:...`  2. consensus -> implement / converge -> fresh round / escalate -> reflector or human decision",
        "reflector": "1. reflector emits `META_RESOLVED:<kind>`  2. controller routes by kind (retry-fix / re-design / re-cluster / drop / escalate-human)",
        "audit": "1. audit emits marker `AUDIT_DONE:...:<N>`  2. controller verifies + opens design issues + dispatches implement",
    },
    "zh": {
...
def build_status_banner(request: BannerRequest, language: HostWorkLanguage = "en") -> str:
    ...
    if request.role not in ROLE_NEXT_STEPS[language]:
        raise ValueError(f"unsupported banner role: {request.role}")
    role = request.role
    next_step = ROLE_NEXT_STEPS[language][role]
...
{AUTO_LOOP_SENTINEL}
""" if language == "zh" else f"""## 📊 Status card - {role} dispatched

| Dimension | Value |
|---|---|
| Stage | **dispatched codex(role=`{role}`)** |
| codex log | `{log_name}` |
| total wall-clock timeout | {request.stall}s(~{request.stall // 60} min) |
| Context | {request.detail or "(none)"} |
| Next automatic action | {next_step} |
