...
3. **不越权扩展范围**：仅实施 source issue、consensus artifact 和 `scope_paths` 授权的当前 work-unit；issue intake/design-consensus 已授权的 feature、bug、doc、refactor、governance 工作可在范围内推进。未请求扩展仍必须先打印 `SCOPE_EXTEND: <file> <reason>`，新接口、新 flag、新模块也只有在当前 work-unit 明确授权时才可引入。新增极小辅助类型的注释也必须遵守 `${HOST_REFACTOR_COMMENT_POLICY}`：missing/empty/default/`none` 时不得写 `refactor helper`, `no behavior change`, `Old`, `New`, 或 `iterN` 等 refactor-history source comments；如确需源码注释，只写面向业务行为的准确英文说明。仅 explicit `self-doc-comment` 时才按第 2 条既有 Refactor self-documentation 格式写注释。
...
10. **IssueDecompositionPlan boundary**：若 consensus 要求 issue decomposition,implement worker 只能落地/验证 controller-private `IssueDecompositionPlan` schema、child body artifact contract、checked-in active-controller apply helper 和文档/测试；worker 输出、validator 通过、`.refactor-loop/host.env`、prompt body 或第一次 `consensus:decompose` 均不是 apply 授权来源；只有 plan-level judge artifact 结构字段 + validated plan digest/proof + #191 owner + live parent open/tracking + sentinel idempotency 可让 #396 投射 named `controller_action="apply_issue_decomposition_plan"`；不得调用或建议 worker GitHub lifecycle,不得新增 public issue factory,不得让 `wakeup_plan.py` 投射 generic decompose action/status,不得 close/reopen/edit parent issue。
