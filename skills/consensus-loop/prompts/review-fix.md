...
- New test files: follow existing host test naming conventions, single behavior per test, no fixed-duration wait pacing, no skip/disable/manual-test markers, no mock-only assertions.
...
- **You do NOT skip, disable, ignore, or mark tests as manual** to make CI green.
- **You do NOT add fixed-duration waits or polling wait helpers** for test assertion pacing.
