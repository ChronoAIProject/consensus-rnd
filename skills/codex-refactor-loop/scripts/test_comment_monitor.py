...
class CommentMonitorTests(unittest.TestCase):
    ...
    def test_team_comment_stale_takeover_posts_notice_before_reacting(self) -> None:
        monitor = CommentMonitor(self.ctx, interval=1)
        calls: list[list[str]] = []
        ...
        self.assertTrue(any("reactions" in " ".join(call) for call in calls))
        notice_index = calls.index(["post_takeover_notice"])
        reaction_index = next(index for index, call in enumerate(calls) if "reactions" in " ".join(call))
        self.assertLess(notice_index, reaction_index)

    def test_team_comment_stale_takeover_notice_failure_fails_closed(self) -> None:
        monitor = CommentMonitor(self.ctx, interval=1)
        calls: list[list[str]] = []
        stale = OwnershipDecision(True, "stale-takeover", WorkTarget("issue", 42), "other", "maintainer", 4.0)
    ...
    def _assert_pr_comment_ownership_gate(self, decision: OwnershipDecision, should_process: bool, comment_id: str) -> None:
        ...
        if should_process:
            ...
            if decision.reason == "stale-takeover":
                ownership_cls.return_value.post_takeover_notice.assert_called_once_with(decision)
                notice_index = calls.index(["post_takeover_notice"])
                reaction_index = next(index for index, call in enumerate(calls) if "reactions" in " ".join(call))
                self.assertLess(notice_index, reaction_index)
        else:
    ...
    def test_pr_team_comment_stale_takeover_notice_failure_fails_closed(self) -> None:
        stale = OwnershipDecision(True, "stale-takeover", WorkTarget("pr", 42), "other", "maintainer", 4.0)
        monitor = CommentMonitor(self.ctx, interval=1)
        calls: list[list[str]] = []
