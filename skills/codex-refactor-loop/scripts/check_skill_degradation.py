...
class SkillDriftChecker:
    ...
    def checker_is_read_only(self) -> list[Finding]:
        ...
        text = "\n".join(
            line for line in self.read(relative).splitlines()
...
def print_findings(findings: list[Finding], *, as_json: bool) -> None:
    if as_json:
        print(json.dumps({"ok": not findings, "findings": [f.as_dict() for f in findings]}, indent=2, sort_keys=True))
        return
    if not findings:
        print("skill-degradation: ok")
        return
    print(f"skill-degradation: {len(findings)} finding(s)")
    for finding in findings:
        print(f"{finding.severity}: {finding.check}: {finding.path}: {finding.message}")


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run read-only codex-refactor-loop degradation checks")
    parser.add_argument("--static", action="store_true", help="run static checks")
    parser.add_argument("--repo-root", type=Path, default=None, help="repository root; defaults to path discovery")
    parser.add_argument("--json", action="store_true", help="print JSON diagnostics")
    args = parser.parse_args(argv)

    if not args.static:
        parser.error("only --static is supported")
    try:
        repo_root = args.repo_root.resolve() if args.repo_root else discover_repo_root(Path.cwd())
        checker = SkillDriftChecker(repo_root)
        findings = checker.run_static()
        print_findings(findings, as_json=args.json)
        return 1 if findings else 0
    except Exception as exc:
        finding = Finding("checker-error", "<runtime>", str(exc))
        print_findings([finding], as_json=args.json)
        return 2
