...
class RuntimeExceptionAuthorizationSourceTests(unittest.TestCase):
    ...
    def test_closed_label_reconciler_238_preserves_closed_only_terminal_boundary(self) -> None:
        ...
        for required in (
            ...
            "per-CLOSED-item `RuntimeError`",
            "daemon-log-only fail-soft",
            "closed-label-reconciler failed: <kind> #<number> terminal=<label> error=<message>",
            "continue later items in the same tick",
            "stale live-plan `None`",
            "crnd:phase:merged",
            "crnd:phase:closed",
        ...
        for forbidden in (
            ...
            "label bootstrap",
            ".controller-pending-events.log` pending-event",
            "durable alert",
            "generic `gh-label`",
            "generic `gh-edit`",
