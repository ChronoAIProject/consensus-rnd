...
| `$PATROL_INSPECTOR_ENABLE` | optional-noop | patrol-inspector | `false` | false or empty exits 0 with disabled patrol state and no patrol-owned issue publication | patrol-inspector | `test_patrol_authority.py`, `test_host_env_surface_matrix.py` |
...
`skills/codex-refactor-loop/scripts/consensus-rnd-cli restart-daemons` 是 checked-in,host-agnostic restart helper。它维护 `restart.py::DAEMON_COMMANDS` owner-local daemon allowlist 的 singleton wrapper + actor-owned heartbeat lease + helper-private launch fingerprint(`concurrency_monitor`, `comment-monitor`, `codex-progress-reporter`, `dev_sync_daemon`, `phase9_router_daemon`, `closed_label_reconciler`, `wakeup_runner_daemon`)。事实源是 `.refactor-loop/locks/<daemon>.pid`、`.refactor-loop/heartbeats/<daemon>.ts`、`.refactor-loop/locks/<daemon>.fingerprint.json`,以及 helper-private `DaemonProcessInventory`;只有 pid alive、actor-loop heartbeat fresh(`<90s`)、fingerprint current、且同一 owner-local allowlist command 零 duplicate canonical live wrapper 时才 skip,missing/malformed/mismatch fail-closed 并重启对应 wrapper。每次 helper tick 先调用 canonical `consensus-rnd-cli runtime-retention`;不保留兼容 alias。
...
Daemon tick summaries are owner-local diagnostics only: each daemon may format its own local
tick/action status for logs or pending events, but those summaries are not a shared production
`TickOutcome`, registry, index, work-unit envelope, or cross-daemon fact source.
