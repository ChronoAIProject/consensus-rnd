...
Zero-code implementation completion is the only empty-scoped-diff closeout carveout: `wakeup-plan`, `wakeup-runner`, and `ControllerActions.close_managed_item_from_drop_marker` must all revalidate consensus judge `scope_paths: none`, worker summary/body no-op proof, clean `IMPLEMENT_DONE:ok`, empty scoped diff, live OPEN managed issue, and #191 owner before closing through the existing #396 helper.
