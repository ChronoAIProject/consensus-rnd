...
Authorization source: `.refactor-loop/runs/phase9-issue66-r8-judge.md`; delete framing preserves mechanical detection while rejecting standalone watchdogs, protocol frameworks, registries, new event envelopes, source mutation, auto-fix, codex dispatch, and GitHub lifecycle surfaces. `check_skill_degradation.py` is the single checker file; CI runs `<skill-root>/scripts/check_skill_degradation.py --static` as required `skill-degradation`, and `auto_release_gate.py` requires it beside `contract-tests` and `manifest-version-sync`. **Narrow allowlist**: run `check_skill_degradation.py`; write `.refactor-loop/.degradation-alert.log`; append existing-format pending events to `.refactor-loop/.controller-pending-events.log`; expose read-only `peek.sh` status. **Forbidden actions**: source mutation, git reset/rebase/merge/push, GitHub issue/PR/body/label lifecycle mutation, codex dispatch, standalone daemon creation, WorkUnit/schema/envelope changes, protocol/plugin registry, auto-clean root garbage, and auto-fix API. Runtime hook: `concurrency_monitor.py` may run the checker on `$DEGRADATION_WATCH_INTERVAL_SECONDS`; failures are alert-only, passing writes nothing, and the hook must not change source files, run git, call GitHub lifecycle APIs, spawn codex, create a new daemon, or change WorkUnit/event schema. Detailed alert format and host config live in [skill degradation watch details](REFERENCE.md#skill-degradation-watch-details).
