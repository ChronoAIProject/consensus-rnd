...
Runtime failure handling: per-CLOSED-item `RuntimeError` from apply or post-edit verify is daemon-log-only fail-soft: print `closed-label-reconciler failed: <kind> #<number> terminal=<label> error=<message>` and continue later items in the same tick; stale live-plan `None` remains the existing skip path.
