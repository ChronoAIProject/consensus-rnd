...
- failure_handling: per-CLOSED-item `RuntimeError` from apply or post-edit verify is daemon-log-only fail-soft: print `closed-label-reconciler failed: <kind> #<number> terminal=<label> error=<message>` and continue later items in the same tick; stale live-plan `None` remains the existing skip path.
- forbidden: no open item mutation, issue create/close/reopen/body/title edit, PR create/merge/close/body/title edit, human label mutation, triage label mutation, milestone label mutation, lifecycle label mutation beyond removing `crnd:lifecycle:stuck`, tag, release, label bootstrap, `.controller-pending-events.log` pending-event, durable alert, generic `gh-label`, generic `gh-edit`, controller close-path inline reconcile, or generic lifecycle actor.
...
- no_new_runtime_authority: This mirror documents only the #238 closed managed item terminal phase-label reconciliation carveout and daemon-log-only fail-soft diagnostics; it does not grant generic GitHub label/edit, label bootstrap, pending-event, durable alert, or lifecycle authority.
