...
- fact_source: design-consensus/meta-reflector consensus artifact plus checked-in `IssueDecompositionPlan` JSON/Markdown body artifacts validated by `issue_decomposition.py`; #396 closed action projection fields are derived only from the clean plan-level judge artifact structured fields, not solver artifacts or prompt-body free text; GitHub child/comment sentinel is the durable idempotency owner; discovery uses phase9-router fallback pending events, generic completed-marker projection, and read-only `peek` pending-events tail.
