...
- allowed: active-controller owner only after `ReleasePublishPreflight` validates `RELEASE_AUTO_ENABLE=true`, fresh `.refactor-loop/state/release-candidate.json`, fresh `.refactor-loop/state/release-decision.json`, matching `decision_digest`, matching `target_ref`, mapped manifest `from_version`, matching coordinate policy when present, mandatory `coordinate_policy.transition=beta_core_promotion` evidence for beta core promotion, and required checks green; first-run exact command allowlist is `python3 .github/scripts/bump_version.py --version <to_version>`, `git add .version-bump.json <mapped manifests>`, `git commit -m "Release v<to_version>"`, `git rev-parse HEAD`, `git fetch origin HEAD`, `git rev-list --count HEAD..origin/HEAD`, `git push origin HEAD`, `gh api repos/<slug>/commits/<fresh release commit sha>/check-runs --paginate --slurp`, and `gh release create v<to_version> --target <fresh release commit sha> --generate-notes [--prerelease]`; already-bumped reentry is allowed only when the only preflight mismatch is mapped manifests already equal `to_version` and `git show -s --format=%s HEAD` proves the HEAD subject is exactly `Release v<to_version>`; remote already-bumped reentry may instead run only `git fetch origin <INTEGRATION_BRANCH>`, `git rev-parse origin/<INTEGRATION_BRANCH>`, `git show -s --format=%s origin/<INTEGRATION_BRANCH>`, and `git show origin/<INTEGRATION_BRANCH>:<mapped manifest>` to prove the remote subject is exactly `Release v<to_version>` and all remote mapped manifests equal `to_version`; if the integration branch head is not that exact release commit, the same remote reentry may additionally read `git for-each-ref --format=%(refname:short) refs/remotes/origin/rollup`, `git rev-parse origin/rollup/<40hex>`, `git show -s --format=%s origin/rollup/<40hex>`, and `git show origin/rollup/<40hex>:<mapped manifest>` only for refs whose suffix equals the resolved commit sha, proving subject exactly `Release v<to_version>` and all mapped manifests equal `to_version`; local reentry may skip only `python3 .github/scripts/bump_version.py --version <to_version>`, `git add .version-bump.json <mapped manifests>`, and `git commit -m "Release v<to_version>"`, then must run `git rev-parse HEAD`, `git fetch origin HEAD`, `git rev-list --count HEAD..origin/HEAD`, `git push origin HEAD`, `gh api repos/<slug>/commits/<exact release/reentry commit sha>/check-runs --paginate --slurp`, and `gh release create v<to_version> --target <exact release/reentry commit sha> --generate-notes [--prerelease]`; remote reentry skips local bump/add/commit/push and must run `gh api repos/<slug>/commits/<remote integration or rollup release sha>/check-runs --paginate --slurp` and `gh release create v<to_version> --target <remote integration or rollup release sha> --generate-notes [--prerelease]`; pending/red/missing/API-fail fail closed before release creation, and `remote_release_reentry_unavailable` also fails closed before release creation; write `.refactor-loop/state/release-publish-result.json`.
...
- allowed: active-controller owner only; read release candidate/decision artifacts, run `ReleasePublishPreflight`, bump mapped manifests and commit/push the release manifest commit, or use already-bumped reentry only after `git show -s --format=%s HEAD` proves the exact release subject, or read-only remote proof shows `origin/<INTEGRATION_BRANCH>` or `origin/rollup/<40hex>` whose suffix equals the resolved commit sha has subject exactly `Release v<to_version>` and mapped manifests equal `to_version`; read exact-SHA Checks API for the exact release/reentry commit sha, create tag/release only after that exact fresh SHA is green, write `.refactor-loop/state/release-publish-result.json`.
...
- fact_source: release candidate/decision artifacts + mapped manifests + exact fresh SHA or read-only proven remote integration/rollup release SHA + Checks API projection, with coordinate policy validated by #322 preflight when applicable.
...
- allowed: active-controller owner only; consume only `wakeup-plan` evidence-bound closed action projection with top-level `mode: "closed-action-projection"`, `no_lifecycle_authority: true`, and `apply_authority: "wakeup-runner-396-only"`; the effect-adapter boundary is only the owner-local admission contract before existing #396 controller actions/named helpers and the #403 `ControllerActions.apply_issue_decomposition_plan()` helper, not a generic effect-adapter runtime abstraction; each executable action must carry `runner_authority: "wakeup-runner-396"`, `preconditions`, `source_marker` or `source_artifact`, `target_kind`, `target_number`, `target`, concrete `controller_action`, and `no_generic_command: true`; effects are allowed only by concrete `controller_action` or helper name; revalidate durable artifact evidence, clean `EXIT=0` source marker, ConsensusGate/meta-judge or review truth table `reject==0 && approve>=1 && all required reviewers present && all required reviewer heads equal live PR head`, OPEN/live GitHub state, missing/stale per-reviewer head SHA, #191 owner, release #322 preflight, or helper-specific precondition; ordinary rejection uses grep-able one-line runner diagnostics such as `WAKEUP_RUNNER_BLOCKED:<action_id>:<reason>` plus ledger/pending-event reason; multi-step external side effects require helper-owned durable result/diagnostic artifacts when that helper owns such a surface or partial external state can exist; `.refactor-loop/host.env` may be skill-private runtime/cache/log read state only, not branch topology, machine paths, durable ledger authority, host artifact, or host production SSOT; `dispatch_consensus_implementation` must create canonical worktree/head, empty reservation commit, push the head, and open the managed draft PR before implement spawn, with missing reservation for already-clean implementation treated as `early_pr_missing` fresh redispatch; `publish_implementation_output` may only commit/push to exactly one matching open managed PR whose head/base/single linked managed issue match, and zero/multiple/mismatch or stale-base clean output fails closed/status-only; clean `IMPLEMENT_DONE:ok` with an empty scoped diff remains status-only unless `wakeup-plan`, `wakeup-runner`, and `ControllerActions.close_managed_item_from_drop_marker` all revalidate the narrow zero-code completion proof: consensus judge `scope_paths: none`, worker summary/body no-op proof, clean source marker, empty scoped diff, live OPEN managed issue, and #191 owner; `wakeup-plan` action `head_sha` is not reviewer-head authority; mechanically call existing controller helpers or #396 narrow helpers for spawn codex, including allowlisted `release-rollup-body` generation that only writes `.refactor-loop/runs/release-rollup-pr-body.md`, named helper `dispatch_consensus_implementation`, named helper `publish_implementation_output`, named helper `apply_issue_decomposition_plan`, named helper `apply_default_issue_intake_claim`, then named helper `open_release_rollup_pr_from_action` after the body exists, publish worker output, dispatch reviewers/fix/remote-ci worker, apply triage decision, merge PR under review truth table, close managed item from drop marker or this revalidated zero-code implementation completion, and publish release through #322.
...
- allowed: active-controller owner only; checked-in `patrol-inspector` runs only after `$PATROL_INSPECTOR_ENABLE=true`; read worker terminal failure envelopes from local logs, runs artifacts, wakeup-plan/peek projections, and GitHub managed item snapshot; raw log prose is diagnostic text, not an issue-intake fact source; generate structured `PatrolFinding`; create or update patrol-owned managed design issues by durable fingerprint; create may attach only the fixed patrol/design-intake label bundle (`crnd:lifecycle:managed`, `crnd:phase:design-solving`, `crnd:human:auto`, `crnd:triage:pending`); update may edit only the patrol issue body; write `.refactor-loop/state/patrol-inspector.json` as cache and #504 dashboard input only.
...
- fact_source: `PatrolFinding.fingerprint` and the patrol fingerprint line in GitHub issue title/body are the idempotency fact source; `.refactor-loop/state/patrol-inspector.json` is cache/dashboard input only and not authorization.
