...
Source: `## Worker Delegation` — the carriers, the dispatch-time layout, flight records,
...
-- SKILL[policy]: "Protocol policy, not a mathematical consequence: at dispatch time, every multi-seat stage assigns exactly one seat to `isolated-token-subagent`, exactly one seat to `nyxid-oracle`, and every remaining seat to `codex-cli`, and the three-seat `## Termination Gate` follows that same layout; every single-worker stage assigns its worker to `codex-cli`."
def defaultLayout (seats : Nat) : List Carrier :=
  if seats ≤ 1 then List.replicate seats .codexCli
  else [.isolatedTokenSubagent, .nyxidOracle] ++ List.replicate (seats - 2) .codexCli

theorem layout_one_subagent_one_oracle (seats : Nat) (h : 2 ≤ seats) :
    (defaultLayout seats).count .isolatedTokenSubagent = 1 ∧
      (defaultLayout seats).count .nyxidOracle = 1 ∧
      (defaultLayout seats).count .codexCli = seats - 2 := by
  have hgt : ¬ seats ≤ 1 := by omega
  simp [defaultLayout, hgt, List.count_cons, List.count_replicate]

theorem single_worker_is_codex : defaultLayout 1 = [.codexCli] := rfl
...
-- SKILL[def]: "This is the default dispatch-time layout; the numbered `WorkerMode` list governs only fallback after a carrier failure."
def governs : DispatchPhase → GoverningRule
  | .dispatchTime => .defaultLayout
  | .afterCarrierFailure => .priorityList

-- SKILL[thm]: "The recorded initial pairing must not be rebalanced in response to completion outcomes; a retry or fallback may replace only the failed flight for the same seat and role, and neither is a mechanism for restoring the default layout."
...
-- SKILL[def]: "A `tests` review seat must be assigned to a carrier capable of executing repository verification commands in the `work_target`."
...
-- SKILL[thm]: "It never covers a whole stage because the `nyxid-oracle` and `isolated-token-subagent` seats reserved by the dispatch-time layout above remain outside the batch."
theorem batch_never_covers_whole_stage (seats : Nat) (h : 2 ≤ seats) :
    (batchSeats (defaultLayout seats)).length < (defaultLayout seats).length := by
  have hgt : ¬ seats ≤ 1 := by omega
  simp [batchSeats, defaultLayout, hgt, List.filter_replicate]
