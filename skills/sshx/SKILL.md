...
An objection that fails `BlockingAuthority` is not an unclosed `GoalArtifact` goal gap and does not by itself hold the exit out of `implement`: the meta-judge records it as advisory in the existing `finding_downgrades` record together with what the objecting seat itself named, or that it named none, using the objecting seat's own words and never a paraphrase. Disputed grounding stays blocking. This is not permission to set aside a reachable defect.
