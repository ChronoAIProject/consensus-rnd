...
Do not use this skill for routine one-step answers where no separate perspectives would change the outcome, and do not run it when the decision's stakes cannot justify the protocol's cost: protocol weight follows decision risk, not available budget.
...
- rabbit-holing into analysis depth, premise-chasing, and advisory volume that changes no `GoalArtifact`-named decision even when nothing blocks done, mirroring the contract's exhaustive register as the required granularity of every judgment;
