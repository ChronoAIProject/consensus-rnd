...
- **controller 纯编排**:controller 只 route / post / label / spawn / commit / push / merge;实现 / 验证 / 修复 / review / design solve 全部 delegate 给 agent worker。跨设备时 GitHub/已 push git 面只允许一个 active-controller lease owner 执行 controller 写路径;本地 `.refactor-loop` 只是 owner 机器缓存/日志,不得授权跨设备工作。可机械化、状态确定的部分可经多角度共识授权后下沉为 narrow allowlist daemon。
